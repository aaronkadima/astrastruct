...
export function BucklingPanel({project,onClose,onVisualize,onCommit}:{project:any;onClose:()=>void;onVisualize:(view:any|null)=>void;onCommit:(project:any)=>void}){
  ...
  const applyImperfection=(target:'pdelta'|'corotational')=>{
    if(is3D&&target==='corotational'){setError('Co‑rotacional 3D ainda não está disponível. Use a imperfeição modal no P‑Delta 3D v0.29.');return}
  ...
  return <div className="react-modal-backdrop" onMouseDown={e=>{if(e.target===e.currentTarget)onClose()}}>
    <section data-testid="panel-buckling" className="react-modal wide buckling-panel" role="dialog" aria-modal="true" aria-label="Estabilidade e flambagem linear">
      ...
      <div className="panel-warning">λcr multiplica o padrão de cargas de referência. Não é coeficiente de segurança nem verificação normativa. {is3D?'A v0.29 permite transferir o modo espacial selecionado como imperfeição geométrica para o P‑Delta 3D; co‑rotacional 3D permanece fora do escopo.':'A flambagem 2D v0.11 aceita frame2d com extremidades rígidas; a imperfeição modal segue esse escopo.'}</div>
