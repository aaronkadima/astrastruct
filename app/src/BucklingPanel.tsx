...
export function BucklingPanel({project,onClose,onVisualize,onCommit}:{project:any;onClose:()=>void;onVisualize:(view:any|null)=>void;onCommit:(project:any)=>void}){
  const scenarios=[...(project.loadCases||[]),...(project.loadCombinations||[])],existing=project.settings?.imperfection;
  ...
  const calculate=()=>{
    try{setRunning(true);setError('');const r=solveBuckling2D(project,scenarioId,{modes:modeCount});const nextIndex=Math.min(selected,Math.max(0,r.modes.length-1));setResult(r);setSelected(nextIndex);onVisualize({result:r,modeIndex:nextIndex})}
  ...
  const applyImperfection=()=>{
    ...
    const mode=result.modes[selected],p=clone(project);p.settings={...(p.settings||{}),analysisType:'pdelta',imperfection:{enabled:true,source:'bucklingMode',scenarioId,mode:mode.mode,amplitudeMm}};onCommit(p);setError('');
  };
  ...
  const active=project.settings?.imperfection?.enabled;
  return <div className="react-modal-backdrop" onMouseDown={e=>{if(e.target===e.currentTarget)onClose()}}>
    <section data-testid="panel-buckling" className="react-modal wide buckling-panel" role="dialog" aria-modal="true" aria-label="Estabilidade e flambagem linear">
      ...
      {active&&<div data-testid="imperfection-active" className="panel-warning success-note">Imperfeição ativa no P‑Delta: modo {project.settings.imperfection.mode}, e₀,max = {fmt(project.settings.imperfection.amplitudeMm,2)} mm, referência {project.settings.imperfection.scenarioId||'cenário atual'}.</div>}
