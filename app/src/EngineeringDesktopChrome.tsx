...
type ResultLegendState={field:string;min:number;max:number;unit:string;kind:'none'|'sequential'|'diverging';phase:number;intensity:number;live:boolean;labelCount:number;zeroLabels:boolean};
...
function ResultLegend({maxDisp,shells}:{maxDisp:number;shells:number}){
  const[state,setState]=useState<ResultLegendState>({field:'none',min:0,max:0,unit:'',kind:'none',phase:1,intensity:1,live:false,labelCount:3,zeroLabels:true});
  useEffect(()=>{
    const read=()=>{const canvas=document.querySelector<HTMLElement>('[data-testid="spatial-canvas-3d"]'),field=canvas?.dataset.forceMode||'none',min=Number(canvas?.dataset.resultMapMin||0),max=Number(canvas?.dataset.resultMapMax||0),unit=canvas?.dataset.resultMapUnit||'',kind=(canvas?.dataset.resultMapKind||'none') as ResultLegendState['kind'],phase=Number(canvas?.dataset.resultMapPhase??1),intensity=Number(canvas?.dataset.resultMapIntensity??1),live=canvas?.dataset.resultMapLive==='true',labelCount=Number(canvas?.dataset.diagramLabelCount??3),zeroLabels=canvas?.dataset.diagramZeroLabels!=='false';const next={field,min:Number.isFinite(min)?min:0,max:Number.isFinite(max)?max:0,unit,kind,phase:Number.isFinite(phase)?phase:1,intensity:Number.isFinite(intensity)?intensity:1,live,labelCount:Number.isFinite(labelCount)?labelCount:3,zeroLabels};setState(current=>current.field===next.field&&current.min===next.min&&current.max===next.max&&current.unit===next.unit&&current.kind===next.kind&&current.phase===next.phase&&current.intensity===next.intensity&&current.live===next.live&&current.labelCount===next.labelCount&&current.zeroLabels===next.zeroLabels?current:next)};
    read();const root=document.querySelector('.astra-app');if(!root)return;const observer=new MutationObserver(read);observer.observe(root,{subtree:true,childList:true,attributes:true,attributeFilter:['data-force-mode','data-result-map-min','data-result-map-max','data-result-map-unit','data-result-map-kind','data-result-map-phase','data-result-map-intensity','data-result-map-live','data-diagram-label-count','data-diagram-zero-labels']});return()=>observer.disconnect();
  },[]);
  const barField=new Set(['N','Vy','Vz','V','T','MyBar','MzBar','M','UxBar','UyBar','UzBar','UmagBar','epsX','kappaY','kappaZ','twistRate']).has(state.field),active=state.field!=='none'&&state.kind!=='none',factor=state.unit==='m'?1000:1,unit=state.unit==='m'?'mm':state.unit,min=state.min*factor,max=state.max*factor,ticks=Array.from({length:9},(_,index)=>max-(max-min)*index/8),intensity=Math.max(0,Math.min(1,state.intensity)),neutral=[235,241,246],gradient=state.kind==='diverging'?`linear-gradient(to bottom,${legendBlend(neutral,[199,71,77],intensity)} 0%,rgb(235,241,246) 50%,${legendBlend(neutral,[92,150,204],intensity)} 100%)`:'linear-gradient(to bottom,rgb(235,77,85) 0%,rgb(55,132,190) 100%)',animatedField=/^U(?:x|y|z|mag)$/.test(state.field);
  ...
  const changeLabelCount=(value:number)=>{const core=document.querySelector<HTMLSelectElement>('select[aria-label="Quantidade de valores permanentes por barra"]');if(core){core.value=String(value);core.dispatchEvent(new Event('change',{bubbles:true}))}};
  const changeZeroLabels=(checked:boolean)=>{const core=document.querySelector<HTMLInputElement>('[data-testid="spatial3d-diagram-zero-labels"]');if(core&&core.checked!==checked){core.checked=checked;core.dispatchEvent(new Event('change',{bubbles:true}))}};
  return <>
    ...
    {barField&&<div className="engineering-diagram-label-controls" data-testid="engineering-diagram-label-controls"><label>Valores/barra <select data-testid="engineering-diagram-label-count" aria-label="Valores permanentes por barra" value={state.labelCount} onChange={e=>changeLabelCount(Number(e.target.value))}><option value="0">0</option><option value="1">1</option><option value="3">3</option><option value="5">5</option><option value="7">7</option></select></label><label><input data-testid="engineering-diagram-zero-labels" type="checkbox" checked={state.zeroLabels} onChange={e=>changeZeroLabels(e.target.checked)}/> Zeros</label></div>}
