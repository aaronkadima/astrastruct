...
export function IfcExchangeShell({children}:{children:React.ReactNode}){
  ...
  const canImport=!!preview&&!busy&&!!preview.readiness?.geometryReady;
  ...
  const inspectFile=(file:File)=>{setBusy(true);setMessage('');const reader=new FileReader();reader.onload=()=>{try{const staged=createIfcImportStaging(String(reader.result));setPreview(staged);setMessage(`IFC preparado: ${staged.summary.nodes} nós · ${staged.summary.elements} elementos · geometria ${staged.readiness.geometryReady?'READY':'PENDING'} · análise ${staged.readiness.analysisReady?'READY':'PENDING'}.`)}catch(e:any){setPreview(null);setMessage(e?.message||String(e))}finally{setBusy(false)}};reader.onerror=()=>{setPreview(null);setBusy(false);setMessage('Não foi possível ler o arquivo IFC selecionado.')};reader.readAsText(file)};
  ...
  return <>
    ...
    {open&&<div className="ifc-exchange-backdrop" role="presentation" onMouseDown={e=>{if(e.target===e.currentTarget)setOpen(false)}}>
      <section className="ifc-exchange-panel" role="dialog" aria-modal="true" aria-label="Intercâmbio IFC4X3" data-testid="ifc-exchange-panel">
        <header><div><strong>Intercâmbio IFC4X3</strong><span>v0.46 import staging · ISO 16739-1:2024</span></div><button aria-label="Fechar intercâmbio IFC" onClick={()=>setOpen(false)}>×</button></header>
        <div className="ifc-exchange-body">
          ...
          {preview&&<div className="ifc-preview" data-testid="ifc-preview">
            <b>Pré-importação estrutural IFC</b><span>{preview.project.name}</span><span>{preview.summary.nodes} nós · {preview.summary.elements} elementos · {preview.summary.materials} materiais · {preview.summary.sections} seções</span>
            <div className="ifc-preview-readiness"><span>Geometria <b className={preview.readiness.geometryReady?'ok':'pending'}>{preview.readiness.geometryReady?'READY':'PENDING'}</b></span><span>Análise <b className={preview.readiness.analysisReady?'ok':'pending'}>{preview.readiness.analysisReady?'READY':'PENDING'}</b></span></div>
            {!!preview.readiness.analysisIssues?.length&&<div className="ifc-import-issues">{preview.readiness.analysisIssues.slice(0,8).map((x:any,i:number)=><span key={`${x.code}-${i}`}>{x.code}: {x.message}</span>)}</div>}
            <small>O arquivo ainda não substituiu o projeto aberto. A importação só ocorre após confirmação explícita e cria um backup local do projeto atual.</small>
            <div className="ifc-exchange-actions"><button className="primary" data-testid="ifc-import-button" disabled={!canImport} onClick={importIfc}>{preview.readiness.analysisReady?'Importar projeto IFC':'Importar geometria (análise pendente)'}</button></div>
          </div>}
          {message&&<div className="ifc-exchange-message" role="status">{message}</div>}
          <div className="ifc-exchange-note">A exportação preserva GlobalIds por projeto. A pré-importação valida unidades, topologia, perfis, apoios/molas e propriedades mecânicas antes de permitir a substituição do modelo.</div>
        </div>
      </section>
    </div>}
  </>;
