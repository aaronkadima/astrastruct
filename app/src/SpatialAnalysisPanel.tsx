...
function spatialIssues(project:any,mode:SpatialMode){
  const issues:string[]=[],elements=project.elements||[],frameOnly=elements.length>0&&elements.every((e:any)=>e.type==='frame3d'),barSpatialOnly=elements.length>0&&elements.every((e:any)=>['frame3d','truss3d'].includes(e.type)),linearSpatialOnly=elements.length>0&&elements.every((e:any)=>['frame3d','truss3d','shell4'].includes(e.type));
  if(!elements.length)issues.push('O modelo não possui elementos.');
  if(mode==='linear'&&!linearSpatialOnly&&elements.length)issues.push('Linear 3D aceita frame3d, truss3d e shell4.');
  if(mode==='modal'){
    if(!barSpatialOnly)issues.push('Modal 3D aceita apenas frame3d e truss3d; shell4 ainda não possui matriz de massa modal nesta versão.');
...
export function SpatialAnalysisPanel({project,onClose,onCommit}:Props){
  ...
  return <div className="react-modal-backdrop" onMouseDown={e=>{if(e.target===e.currentTarget)onClose()}}>
    <section data-testid="panel-spatial-analysis" className="react-modal" role="dialog" aria-modal="true" aria-label="Tipo de análise espacial 3D">
      ...
      {hasShell&&<div data-testid="shell4-analysis-scope" className="panel-note"><b>Shell4 ativo no modelo:</b> membrana + flexão Mindlin–Reissner + cisalhamento transversal e pressão superficial estão disponíveis no <b>Linear 3D v0.30</b>. Modal, P‑Delta e co‑rotacional de cascas entram em etapas posteriores.</div>}
