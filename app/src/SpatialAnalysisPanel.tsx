...
function spatialIssues(project:any,mode:SpatialMode){
  ...
  if(mode==='corotational'){
    ...
    const allowed=new Set(['uniform','point','selfWeight','thermal']),bad=(project.elementLoads||[]).filter((l:any)=>!allowed.has(l.kind));if(bad.length)issues.push(`Co‑rotacional 3D: cargas de barra ainda não suportadas: ${[...new Set(bad.map((x:any)=>x.kind||'desconhecida'))].join(', ')}.`);
  }
