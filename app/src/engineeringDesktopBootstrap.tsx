...
function ensureHost(parent:Element,cls:string,where:'prepend'|'append'|'after'='prepend'){
  let host=parent.querySelector<HTMLElement>(`:scope > .${cls}`);if(host)return host;host=document.createElement('div');host.className=cls;if(where==='prepend')parent.prepend(host);else parent.append(host);return host;
}
...
function renderAll(){const app=document.querySelector<HTMLElement>('.astra-app');if(!app)return;app.classList.add('engineering-desktop');project=project||readProject();result=result??currentAnalysisResult();
  ...
  if(right){const h=ensureHost(right,'engineering-right-host');rootFor('right',h).render(<EngineeringRightRail project={project} result={result} onOpenPanel={openPanel}/>)}
