...
function registrationSignature(reg:ReturnType<typeof getIsolatedLabs>[number]){
  return JSON.stringify([reg.id,reg.title,reg.description,reg.meta,reg.order??100,reg.testId??'']);
}

function renderRegistry(overlay:Element){
  const host=isolatedLabHost(overlay);if(!host)return;
  const registrations=getIsolatedLabs(),wanted=new Set(registrations.map(r=>r.id));
  ...
  for(const reg of registrations){
    let card=host.querySelector<HTMLElement>(`[data-registered-isolated-lab="${reg.id}"]`),created=false;
    if(!card){card=document.createElement('article');card.className='astra-model-card';card.dataset.registeredIsolatedLab=reg.id;created=true}
    const signature=registrationSignature(reg);
    if(card.dataset.registrySignature!==signature){
      card.dataset.registrySignature=signature;
    ...
    if(created)host.appendChild(card);
  }
}
