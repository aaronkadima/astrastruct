...
function patchLabStatus(){
  document.querySelectorAll<HTMLElement>('[data-testid="model-lab-overlay"]').forEach(overlay=>{
    ...
    if(!note||note.dataset.capabilitiesV0304==='1')return;
    note.dataset.capabilitiesV0304='1';
    note.innerHTML='<b>Lab v0.30.4:</b> vigas, pilares e barras isoladas usam os solvers estruturais do AstraStruct. <b>Ancoragem / pull-out</b> possui aderência τ–s com pós-pico e mapa espacial de τ na interface aço–concreto. <b>Ligação chapa–parafuso</b> inclui distribuição elástica Fx/Fy/Mz, contato não linear em furo circular com <b>folga radial</b>, <b>furo oblongo orientável</b> e, agora, <b>chapa flexível Q4</b> em estado plano de tensões com bearing unilateral e plastificação bilinear governada por σvm. A inspeção de contato mantém σb,eq=V/(t·d) como tensão média equivalente. <b>Punção</b> possui demanda V/Mx/My no perímetro crítico, curva τ(s) e mapa contínuo em planta. Resistências normativas, pressão Hertziana/FEM local no contorno do furo, block shear, rasgamento de borda, prying fora do plano e ruptura de cone/borda de concreto permanecem como módulos separados em desenvolvimento.';
  });
}
