  ...
  function renderBuilding(){
    ...
    const bindForm=(prefix='g')=>{
      const sync=()=>{
        ...
        const diaphragm=control(prefix,'diaphragm') as HTMLSelectElement|null;if(diaphragm){const semi=[...diaphragm.options].find(o=>o.value==='semiRigid'),rigid=[...diaphragm.options].find(o=>o.value==='rigid'),corotational=analysis?.value==='corotational';if(semi)semi.disabled=!slabs;if(rigid)rigid.disabled=!!corotational;if(!slabs&&diaphragm.value==='semiRigid')diaphragm.value=corotational?'none':'rigid';if(corotational&&diaphragm.value==='rigid')diaphragm.value='none'}
        const diaphragmNote=control(prefix,'diaphragm-note');if(diaphragmNote)diaphragmNote.textContent=analysis?.value==='corotational'?'Co-rotacional 3D atual: diafragma rígido MPC ainda não participa do Newton; use sem diafragma rígido.':diaphragm?.value==='rigid'?'Rígido XY: nós do pavimento compartilham UX, UY e RZ de corpo rígido; UZ/RX/RY continuam independentes.':diaphragm?.value==='semiRigid'?'Semi-rígido: a rigidez em planta vem dos elementos shell4 da laje.':'Sem MPC de pavimento; use somente quando a conectividade estrutural representar o comportamento desejado.';
