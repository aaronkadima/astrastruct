...
function applyLayout(state=readLayout()){
  ...
  app.classList.toggle('ui-hide-left',!state.left);
  app.classList.toggle('ui-hide-right',!state.right);
  app.classList.toggle('ui-hide-results',!state.results);
  app.classList.toggle('ui-canvas-focus',state.focus);
  app.classList.toggle('canvas-white',state.canvas==='white');
  app.classList.toggle('canvas-black',state.canvas==='black');
  ...
  document.querySelectorAll<HTMLElement>('[data-layout-toggle]').forEach(b=>{
    const key=b.dataset.layoutToggle as keyof LayoutState;
    const on=key==='focus'?state.focus:Boolean(state[key]);
    b.classList.toggle('active',!!on);b.setAttribute('aria-pressed',String(!!on));
  });
  document.querySelectorAll<HTMLElement>('[data-canvas-theme]').forEach(b=>{
    const on=b.dataset.canvasTheme===state.canvas;
...
function openFile(file:File){
  ...
  r.onload=()=>{try{const raw=JSON.parse(String(r.result||''));const project=raw?.format==='AstraStruct NZ'&&raw.project?raw.project:raw;if(!project||!Array.isArray(project.nodes)||!Array.isArray(project.elements))throw new Error('estrutura de projeto inválida');localStorage.setItem(STORAGE_KEY,JSON.stringify(project));location.reload()}catch(e:any){alert(`Não foi possível abrir o arquivo: ${e?.message||e}`)}};
  r.readAsText(file);
}

function visible<T extends Element>(nodes:NodeListOf<T>|T[]){return [...nodes].find(el=>{const r=el.getBoundingClientRect(),s=getComputedStyle(el);return r.width>1&&r.height>1&&s.display!=='none'&&s.visibility!=='hidden'})||null}
...
function scientificSvg(){
  const charts=document.querySelectorAll<SVGSVGElement>('.react-modal svg.react-chart');
  const source=visible(charts)||visible(document.querySelectorAll<SVGSVGElement>('svg.model-canvas'));
  ...
  const box=source.viewBox?.baseVal,rect=source.getBoundingClientRect();
  const vb=box&&box.width>0?`${box.x} ${box.y} ${box.width} ${box.height}`:`0 0 ${Math.max(1,rect.width)} ${Math.max(1,rect.height)}`;
  clone.setAttribute('viewBox',vb);clone.removeAttribute('width');clone.removeAttribute('height');
  const nums=vb.split(/\s+/).map(Number),bg=document.documentElement.dataset.canvasTheme==='black'?'#101113':'#ffffff';
...
function exportScientificSvg(){
  ...
  if(!fig){if(visible(document.querySelectorAll<HTMLCanvasElement>('.spatial3d-canvas')))alert('O Canvas 3D é rasterizado. Use “PNG científico · 600 dpi”.');else alert('Nenhuma figura SVG visível para exportar.');return}
  const p=currentProject(),text='<?xml version="1.0" encoding="UTF-8"?>\n'+new XMLSerializer().serializeToString(fig.svg);
  download(`${slug(p?.name)}-figura-cientifica.svg`,text,'image/svg+xml;charset=utf-8');
}
async function exportScientificPng(){
  const p=currentProject(),targetWidth=4252,theme=document.documentElement.dataset.canvasTheme||'white'; // 180 mm at 600 dpi
  const fig=scientificSvg();
  if(fig){
    const xml=new XMLSerializer().serializeToString(fig.svg),url=URL.createObjectURL(new Blob([xml],{type:'image/svg+xml'})),img=new Image();
    await new Promise<void>((resolve,reject)=>{img.onload=()=>resolve();img.onerror=()=>reject(new Error('Falha ao rasterizar SVG'));img.src=url});
    const canvas=document.createElement('canvas');canvas.width=targetWidth;canvas.height=Math.max(1,Math.round(targetWidth*fig.ratio));const ctx=canvas.getContext('2d')!;ctx.drawImage(img,0,0,canvas.width,canvas.height);URL.revokeObjectURL(url);
    canvas.toBlob(b=>{if(b)saveBlob(`${slug(p?.name)}-figura-cientifica-600dpi.png`,b)},'image/png');return;
  }
  const source=visible(document.querySelectorAll<HTMLCanvasElement>('.spatial3d-canvas'));
  if(!source){alert('Nenhuma figura visível para exportar.');return}
  const ratio=source.height/Math.max(1,source.width),canvas=document.createElement('canvas');canvas.width=targetWidth;canvas.height=Math.max(1,Math.round(targetWidth*ratio));const ctx=canvas.getContext('2d')!;
  ctx.imageSmoothingEnabled=true;ctx.imageSmoothingQuality='high';if(theme==='black')ctx.filter='invert(1) hue-rotate(180deg) brightness(.9) contrast(1.05)';ctx.drawImage(source,0,0,canvas.width,canvas.height);ctx.filter='none';
  canvas.toBlob(b=>{if(b)saveBlob(`${slug(p?.name)}-figura-cientifica-600dpi.png`,b)},'image/png');
}
...
function runAction(action:string,input:HTMLInputElement){
  ...
  if(action==='file:import'){document.querySelector<HTMLInputElement>('.topbar input[type=file]')?.click();return}
...
function closeMenus(topbar:HTMLElement){
  topbar.querySelectorAll('.modern-menu.open').forEach(m=>{m.classList.remove('open');m.querySelector<HTMLElement>('[aria-expanded]')?.setAttribute('aria-expanded','false')});
  const brand=topbar.querySelector<HTMLElement>('.brand.file-menu-open');if(brand){brand.classList.remove('file-menu-open');brand.querySelector<HTMLElement>('.brand-mark')?.setAttribute('aria-expanded','false')}
}
function menuItems(items:readonly (readonly string[])[]){
  return items.map(item=>{
    if(item[0]==='separator')return '<div class="modern-menu-separator" role="separator"></div>';
    const action=String(item[1]),radio=action.startsWith('canvas:'),role=radio?'menuitemradio':'menuitem',canvas=radio?` data-canvas-theme="${action.slice(7)}" aria-checked="false"`:'';
    return `<button role="${role}" data-modern-action="${action}"${canvas}><span>${item[0]}</span>${item[2]?`<kbd>${item[2]}</kbd>`:''}</button>`;
  }).join('');
}
function bindActions(topbar:HTMLElement,input:HTMLInputElement){
  topbar.querySelectorAll<HTMLButtonElement>('[data-modern-action]').forEach(b=>{if(b.dataset.bound)return;b.dataset.bound='1';b.addEventListener('click',()=>{runAction(b.dataset.modernAction!,input);closeMenus(topbar)})});
}
function buildMenu(){
  ...
  if(!bar){
    bar=document.createElement('nav');bar.className='modern-menubar';bar.setAttribute('aria-label','Menus AstraStruct');
    const groups=menuGroups.map(([title,items],i)=>`<div class="modern-menu"><button class="modern-menu-button" data-menu="${i}" aria-haspopup="true" aria-expanded="false">${title}</button><div class="modern-menu-popup" role="menu">${menuItems(items as any)}</div></div>`).join('');
    bar.innerHTML=`<div class="modern-menu-groups">${groups}</div><input class="modern-open-file" type="file" accept=".nz,.json,application/json,application/x-astrastruct+json" hidden>`;
    topbar.insertBefore(bar,topbar.querySelector('.top-actions'));
  }
  const brand=topbar.querySelector<HTMLElement>('.brand'),mark=brand?.querySelector<HTMLElement>('.brand-mark');
  if(brand&&mark&&!brand.querySelector('.modern-file-popup')){
    mark.classList.add('modern-app-menu');mark.setAttribute('role','button');mark.setAttribute('tabindex','0');mark.setAttribute('aria-label','Arquivo');mark.setAttribute('aria-haspopup','true');mark.setAttribute('aria-expanded','false');
    const popup=document.createElement('div');popup.className='modern-menu-popup modern-file-popup';popup.setAttribute('role','menu');popup.innerHTML=menuItems(fileItems as any);brand.appendChild(popup);
    const open=()=>{const should=!brand.classList.contains('file-menu-open');closeMenus(topbar);if(should){brand.classList.add('file-menu-open');mark.setAttribute('aria-expanded','true')}};
    mark.addEventListener('click',e=>{e.stopPropagation();open()});mark.addEventListener('keydown',e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();open()}});
  }
  const input=bar.querySelector<HTMLInputElement>('.modern-open-file')!;
  if(!bar.dataset.bound){
    bar.dataset.bound='1';
    bar.querySelectorAll<HTMLButtonElement>('.modern-menu-button').forEach(b=>b.addEventListener('click',e=>{e.stopPropagation();const menu=b.parentElement!,should=!menu.classList.contains('open');closeMenus(topbar);if(should){menu.classList.add('open');b.setAttribute('aria-expanded','true')}}));
    input.addEventListener('change',()=>{if(input.files?.[0])openFile(input.files[0]);input.value=''});
    document.addEventListener('pointerdown',e=>{const target=e.target as Node;if(!topbar.contains(target))closeMenus(topbar)});
  }
  bindActions(topbar,input);applyLayout();
}
...
function buildUnifiedTools(){
  ...
  let group=topbar.querySelector<HTMLElement>('.modern-unified-tools');
  if(!group){group=document.createElement('div');group.className='modern-unified-tools model-tools';group.setAttribute('aria-label','Ferramentas de modelagem e edição');topbar.querySelector('.modern-menubar')?.insertAdjacentElement('afterend',group)}
  const originals=[...source.querySelectorAll<HTMLButtonElement>('button')];
  const signature=originals.map(b=>`${b.getAttribute('aria-label')}:${b.classList.contains('active')}:${b.disabled}`).join('|');if(group.dataset.signature===signature)return;
  group.dataset.signature=signature;group.innerHTML='';
  originals.forEach((original,index)=>{
    const clone=original.cloneNode(true) as HTMLButtonElement;clone.disabled=original.disabled;clone.classList.add('modern-proxy-tool');if(index===4)clone.classList.add('modern-group-start');
    clone.addEventListener('click',()=>{if(!original.disabled)original.click();setTimeout(schedule,0)});group!.appendChild(clone);
  });
...
function groupPrimaryToolbar(){
  ...
  if(actions){const buttons=[...actions.querySelectorAll<HTMLButtonElement>(':scope > button')];buttons.slice(0,3).forEach(b=>b.classList.add('modern-superseded'));buttons[3]?.classList.add('modern-group-start');buttons[8]?.classList.add('modern-group-start')}
...
function removeRedundancy(){
  document.querySelector<HTMLElement>('.brand')?.querySelector('div')?.classList.add('modern-brand-copy');
...
document.addEventListener('keydown',e=>{
  ...
  else if((e.ctrlKey||e.metaKey)&&key==='n'){e.preventDefault();clickCommand('Novo projeto')}
