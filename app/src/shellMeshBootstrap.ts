...
// @ts-ignore
import { detectQuadrilateralShellCells, addQuadrilateralShellPanels } from '../../web/src/core/shellMesh.js';
...
function patchButton(create:HTMLButtonElement){
  ...
  const count=()=>{const raw=current(),level=$<HTMLSelectElement>('[data-shell-level]',body);if(!raw||!level)return 0;const p=assignLevels(raw);return detectQuadrilateralShellCells(p,{levelId:level.value,requireBoundaryElements:true}).length};
  const refresh=()=>{const n=count();button.textContent=n?`Auto preencher ${n} vão${n===1?'':'s'} Q4`:'Nenhum vão Q4 fechado livre';button.disabled=n===0};refresh();
  $<HTMLSelectElement>('[data-shell-level]',body)?.addEventListener('change',()=>setTimeout(refresh,0));
  button.onclick=()=>{try{const raw=current();if(!raw)throw new Error('Nenhum projeto carregado.');const p=assignLevels(raw),level=$<HTMLSelectElement>('[data-shell-level]',body),material=$<HTMLSelectElement>('[data-shell-material]',body),t=$<HTMLInputElement>('[data-shell-t]',body),pressure=$<HTMLInputElement>('[data-shell-p]',body),kappa=$<HTMLInputElement>('[data-shell-kappa]',body);if(!level||!material||!t||!pressure||!kappa)throw new Error('Controles de laje indisponíveis.');const result=addQuadrilateralShellPanels(p,{levelId:level.value,materialId:material.value,thickness:Math.max(1e-5,num(t.value,.18)),pressure:num(pressure.value),caseId:p.settings?.activeLoadCaseId||p.loadCases?.[0]?.id,shearCorrection:Math.max(.05,Math.min(1,num(kappa.value,5/6))),requireBoundaryElements:true,labelPrefix:'Laje'});save(result.project,result.report)}catch(err:any){alert(err?.message||String(err))}};
}
