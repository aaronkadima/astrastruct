...
export function useProjectHistory(initial:any){
  ...
  useEffect(()=>{persist(projectRef.current);},[]);
  ...
  useEffect(()=>{const handle=(event:Event)=>{const project=(event as CustomEvent)?.detail?.project;if(project)commit(project)};window.addEventListener('astrastruct:project-external-commit',handle);return()=>window.removeEventListener('astrastruct:project-external-commit',handle)},[commit]);

  const undo=useCallback(()=>{const prev=undoRef.current.pop();if(!prev)return;redoRef.current.push(clone(projectRef.current));replace(normalizeProject(prev));},[]);
  const redo=useCallback(()=>{const next=redoRef.current.pop();if(!next)return;undoRef.current.push(clone(projectRef.current));replace(normalizeProject(next));},[]);
