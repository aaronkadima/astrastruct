...
1. **Aparência por grupo estrutural** — configurada diretamente em **Canvas e visualização**, com cor RGB/HEX, opacidade de 0 a 100% e visibilidade por grupos `Lajes`, `Vigas`, `Pilares`, `Paredes`, `Treliças / contraventamentos / cabos`, `Fundações` e `Outros`. A aplicação é coletiva: alterar um grupo atualiza todos os elementos classificados naquele grupo. Não existe nova edição visual individual na v0.52.
...
As configurações por grupo são persistidas em `project.visualization.groupAppearance`. Para compatibilidade com o canvas atual, estilos efetivos também são materializados em `project.visualization.elementAppearance` com `derivedFromGroup`.
