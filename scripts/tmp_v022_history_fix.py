...
insert="""assert(result.materialNonlinearity.maxEquivalentPlasticStrain>0,'global cyclic hinge should accumulate plastic demand');
...
assert(Math.abs(result.pushover.finalLoadFactor-virgin.pushover.finalLoadFactor)>1e-4,'cyclic hinge final equilibrium must depend on committed prior history');
...
"""
if "final equilibrium must depend on committed prior history" not in s:
