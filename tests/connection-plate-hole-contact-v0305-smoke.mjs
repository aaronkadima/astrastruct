...
function probe(cfg,group){const z=solveConnectionPlateHoleAtDisplacement({...cfg,bolts:group},0),f=solveConnectionPlateHoleAtDisplacement({...cfg,bolts:group},.0008,z.u),b=solveConnectionPlateHoleAtDisplacement({...cfg,bolts:group},.0020,f.u);return{Fy:b.totalFy,Fx:b.totalFx,load:b.appliedLoad,converged:b.converged,holes:b.state.holes.map(h=>({id:h.id,Fx:h.resultant.fx,Fy:h.resultant.fy,theta:h.contactCentroidAngleDeg,uy:h.meanBoundaryDisplacement.y}))}}
...
console.log('cut-cell symmetry probes',JSON.stringify({
  q8:probe({...base,cutIntegrationOrder:8},bolts),
  q12:probe({...base,cutIntegrationOrder:12},bolts),
  q16:probe({...base,cutIntegrationOrder:16},bolts),
  single:probe({...base,cutIntegrationOrder:12},single)
}));
...
assert.ok(plastic.events.firstContact,'missing first contact');assert.ok(plastic.events.firstYield,'missing first yield');assert.ok(plastic.final.peakPressureMPa>0);assert.ok(plastic.final.maxOvalizationMm>0);assert.ok(plastic.final.state.holes.every(h=>Number.isFinite(h.bearingStressEquivalentMPa)&&h.segments.every(s=>Number.isFinite(s.pressureMPa))));
