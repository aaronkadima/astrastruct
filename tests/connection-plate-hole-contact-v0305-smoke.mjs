...
function probe(cfg,group){
  const z=solveConnectionPlateHoleAtDisplacement({...cfg,bolts:group},0);
  const f=solveConnectionPlateHoleAtDisplacement({...cfg,bolts:group},.0008,z.u);
  const b=solveConnectionPlateHoleAtDisplacement({...cfg,bolts:group},.0020,f.u);
  return{Fy:b.totalFy,Fx:b.totalFx,load:b.appliedLoad,maxVm:b.maxVmMPa,yielded:b.yieldedAreaFraction,converged:b.converged,holes:b.state.holes.map(h=>({id:h.id,Fx:h.resultant.fx,Fy:h.resultant.fy,theta:h.contactCentroidAngleDeg,uy:h.meanBoundaryDisplacement.y}))};
}
...
console.log('symmetry mechanism probes',JSON.stringify({groupPlastic:probe(base,bolts),groupElastic:probe({...base,fy:1e9},bolts),singlePlastic:probe(base,single),singleElastic:probe({...base,fy:1e9},single)}));
...
console.log('plastic path diagnostic',JSON.stringify({
  elastic:{converged:elastic.diagnostics.converged,failedAt:elastic.diagnostics.failedAt,maxVm:elastic.diagnostics.maxVmMPa,yielded:elastic.diagnostics.yieldedAreaFraction,curve:elastic.curve.map(p=>({s:p.step,c:p.converged,u:p.displacementMm,F:p.load,vm:p.maxVmMPa,y:p.yieldedAreaFraction,it:p.iterations,r:p.residualNorm}))},
  plastic:{converged:plastic.diagnostics.converged,failedAt:plastic.diagnostics.failedAt,maxVm:plastic.diagnostics.maxVmMPa,yielded:plastic.diagnostics.yieldedAreaFraction,curve:plastic.curve.map(p=>({s:p.step,c:p.converged,u:p.displacementMm,F:p.load,vm:p.maxVmMPa,y:p.yieldedAreaFraction,it:p.iterations,r:p.residualNorm}))}
}));
