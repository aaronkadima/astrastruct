...
{
 const p=model([{id:'T',elementId:'E1',kind:'thermal',dT}]),r=solveFrameCorotational3DWithDeadLoads(p,{steps:6,tolerance:1e-9}),u=r.displacements[1].ux,expected=alpha*dT*L;
...
{
 const p=model([{id:'T',elementId:'E1',kind:'thermal',dT}],{ux:true}),r=solveFrameCorotational3DWithDeadLoads(p,{steps:5,tolerance:1e-9}),expected=E*A*alpha*dT,N=r.elementForces[0].N2;
...
{
 const gradient=20,kappa=-alpha*gradient/h,p=model([{id:'TG',elementId:'E1',kind:'thermal',dT:0,dTGradient:gradient}]),r=solveFrameCorotational3DWithDeadLoads(p,{steps:8,tolerance:2e-9}),u=r.displacements[1],expectedY=L*Math.sin(kappa*L/2),expectedRz=kappa*L;
