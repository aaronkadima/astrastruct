...
import {IFC_SCHEMA,IFC_STANDARD,IFC_LOAD_EXCHANGE_CONTRACT,IFC_LOAD_EXCHANGE_VERSION,IFC_MATERIAL_STRENGTH_VERSION,IFC_IMPORT_STAGING_VERSION} from '../web/src/interop/index.js';
...
for(const test of ['tests/development-gate-v048-smoke.mjs','tests/ifc-load-mapping-v048-smoke.mjs','tests/ifc-material-strength-v047-smoke.mjs','tests/ifc-import-staging-v046-smoke.mjs'])assert.ok(pkg.scripts.test.includes(test),`gate obrigatório ausente: ${test}`);
...
for(const phrase of ['IfcStructuralLoadCase','GLOBAL_COORDS','LOCAL_COORDS','IfcStructuralLinearAction','IfcRelAssignsToGroupByFactor','IfcRelConnectsStructuralActivity','Nenhuma promoção para `main`'])assert.ok(docs.includes(phrase),`documentação v0.48 incompleta: ${phrase}`);

console.log('AstraStruct v0.48 development gate: canonical IFC load cases, actions and factored combinations active while product remains formally v0.47.0.');
