...
assert.equal(IFC_MATERIAL_STRENGTH_VERSION,'0.47.0-exp');assert.equal(IFC_IMPORT_STAGING_VERSION,'0.48.0-exp');
...
for(const phrase of ['IfcStructuralLoadCase','GLOBAL_COORDS','LOCAL_COORDS','IfcStructuralLinearAction','IfcRelAssignsToGroupByFactor','IfcRelConnectsStructuralActivity','LoadedBy','loadGlobalIds','ifc-step-load-parse/v1','exchangeStateSeed','commitReady','Nenhuma promoção para `main`'])assert.ok(docs.includes(phrase),`documentação v0.48 incompleta: ${phrase}`);

console.log('AstraStruct v0.48 development gate: canonical, STEP, parser and import staging IFC load exchange active while product remains formally v0.47.0.');
