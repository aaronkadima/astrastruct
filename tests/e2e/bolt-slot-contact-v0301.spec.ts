...
test('isolated Lab resolves oriented slotted bolt-hole contact',async({page},testInfo)=>{
  ...
  const kpis=lab.locator('.slot-contact-kpi strong'),ux=parseFloat((await kpis.nth(2).textContent())||'0');
  expect(Math.abs(ux-7.1)).toBeLessThan(.03);
  ...
  const csvPromise=page.waitForEvent('download');await lab.locator('[data-slot-csv]').click();const csvDownload=await csvPromise,csvPath=await csvDownload.path();expect(csvPath).toBeTruthy();const csv=await readFile(csvPath!,'utf8');expect(csv).toContain('slot_coordinate_mm');expect(csv).toContain('bearing-elastic');
