...
test('v0.23 modal and time-history workflow exposes frequencies, participation and Newmark history',async({page})=>{
  ...
  await openCommand(page,'Diagramas/envelopes');
  const timePost=page.getByTestId('panel-dynamics-postprocess');await expect(timePost).toContainText('Newmark-β');await expect(page.getByTestId('dynamic-time-chart')).toBeVisible();await expect(page.getByTestId('dynamic-peak-displacement')).toBeVisible();await expect(page.getByTestId('dynamic-time-summary')).toContainText('Rayleigh');await expect(timePost).toContainText('vetor de cargas do cenário');await expect(page.locator('[role="alert"]')).toHaveCount(0);
});
