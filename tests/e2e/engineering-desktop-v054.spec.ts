...
test('v0.54 nonlinear 3D results animate the converged load path',async({page})=>{
  ...
  await expect(animate).toBeVisible();await expect(animate).toBeEnabled();await expect(animate).toHaveAttribute('data-animate-mode','load-path');await expect(animate).toHaveText('Animar');
  ...
  await animate.click();
  await expect(canvas).toHaveAttribute('data-animation-playing','true');await expect(animate).toHaveText('Parar');await expect(animate).toHaveAttribute('aria-pressed','true');
  await expect.poll(async()=>Number(await canvas.getAttribute('data-load-path-lambda'))).toBeLessThan(.95);
  await animate.click();
  await expect(canvas).toHaveAttribute('data-animation-playing','false');await expect(animate).toHaveText('Animar');await expect(animate).toHaveAttribute('aria-pressed','false');
