...
test('v0.54 displacement map and result legend follow every animation frame',async({page},testInfo)=>{
  ...
  await expect.poll(async()=>page.evaluate(({finalMax,finalGradient})=>{const canvas=document.querySelector<HTMLElement>('[data-testid="spatial-canvas-3d"]'),legend=document.querySelector<HTMLElement>('[data-testid="engineering-result-legend"]'),phase=Number(canvas?.dataset.resultMapPhase),intensity=Number(canvas?.dataset.resultMapIntensity),maximum=Number(canvas?.dataset.resultMapMax),top=Number(legend?.querySelector('.eng-legend-ticks span')?.textContent),gradient=legend?.querySelector<HTMLElement>('.eng-gradient');return phase>.08&&phase<.82&&intensity>.08&&intensity<.82&&maximum>0&&maximum<finalMax*.85&&legend?.dataset.resultLive==='true'&&Math.abs(Number(legend.dataset.resultPhase)-phase)<.03&&Math.abs(top-maximum*1000)<Math.max(.002,maximum*20)&&getComputedStyle(gradient!).backgroundImage!==finalGradient&&legend.textContent?.includes('Animação')===true},{finalMax,finalGradient})).toBe(true);
