...
test('v0.54 engineering desktop exposes ribbon, model tree, right rail and bottom results',async({page})=>{
  ...
  const app=page.getByTestId('astra-app');await expect(app).toHaveClass(/engineering-desktop/);
...
test('v0.54 3D result publishing keeps shell/foundation visualization contract available',async({page})=>{
  ...
  await page.goto('');await page.waitForFunction(()=>document.documentElement.dataset.astraReady==='true');await page.getByTestId('engineering-ribbon-analyze').click();
