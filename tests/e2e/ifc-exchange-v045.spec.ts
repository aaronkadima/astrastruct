...
test('IFC4X3 exchange panel exports with persistent identities',async({page})=>{
  ...
  expect(firstState.contract).toBe('ifc-exchange-state/v1');expect(Object.keys(firstState.ids).length).toBeGreaterThan(0);expect(Object.keys(firstState.loadGlobalIds||{}).length).toBeGreaterThan(0);
  ...
  expect(secondState.ids).toEqual(firstState.ids);expect(secondState.declarationGlobalId).toBe(firstState.declarationGlobalId);expect(secondState.groupGlobalId).toBe(firstState.groupGlobalId);expect(secondState.materialAssociationGlobalIds).toEqual(firstState.materialAssociationGlobalIds);expect(secondState.loadGlobalIds).toEqual(firstState.loadGlobalIds);expect(secondState.creationDate).toBe(firstState.creationDate);
});

test('IFC staged import validates structure and loads before replacement and creates a local backup',async({page})=>{
  ...
  const preview=page.getByTestId('ifc-preview');await expect(preview).toBeVisible();await expect(preview).toContainText('Geometria READY');await expect(preview).toContainText('Cargas READY');await expect(preview).toContainText('Importação READY');await expect(preview).toContainText('Análise READY');
  ...
  const persisted=await page.evaluate(()=>({project:JSON.parse(localStorage.getItem('astrastruct.project')||'null'),backup:JSON.parse(localStorage.getItem('astrastruct.ifc.import.backup.v046')||'null'),states:Object.entries(localStorage).filter(([k])=>k.startsWith('astrastruct.ifc.v045:')).map(([k,v])=>[k,JSON.parse(v)])}));
  expect(persisted.backup?.project?.id).toBe(original.id);expect(persisted.backup?.sourceProjectId).toBe(original.id);expect(persisted.project?.meta?.importedFrom?.format).toBe('IFC');expect(persisted.project?.meta?.analysisReady).toBe(true);expect(persisted.project?.ifcImport?.loadReady).toBe(true);expect(persisted.project?.ifcImport?.commitReady).toBe(true);expect(persisted.project?.ifcImport?.exchangeStateSeed?.loadGlobalIds).toBeTruthy();expect(persisted.states.some(([,state]:any)=>state?.projectId===persisted.project?.id&&state?.loadGlobalIds)).toBe(true);
});

test('IFC without mechanical properties remains importable but analysis-pending',async({page})=>{
  ...
  const preview=page.getByTestId('ifc-preview');await expect(preview).toBeVisible();await expect(preview).toContainText('Geometria READY');await expect(preview).toContainText('Cargas READY');await expect(preview).toContainText('Importação READY');await expect(preview).toContainText('Análise PENDING');await expect(preview).toContainText('YOUNG_MODULUS_MISSING');
  const importButton=page.getByTestId('ifc-import-button');await expect(importButton).toBeEnabled();await expect(importButton).toHaveText('Importar projeto (análise pendente)');
});
