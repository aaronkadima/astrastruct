...
test('P0 central registry owns all local mechanics Labs without legacy duplicate cards',async({page},testInfo)=>{
  ...
  const cards=overlay.locator('[data-registered-isolated-lab]');await expect(cards).toHaveCount(7);
  ...
  await expect(overlay).toBeVisible();await expect(cards).toHaveCount(7);await page.getByTestId('open-punching-lab').click();await expect(page.getByTestId('punching-lab')).toBeVisible();
});
