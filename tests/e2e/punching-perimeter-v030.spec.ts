...
test('isolated Lab calculates punching demand, exports CSV and scientific SVG',async({page},testInfo)=>{
  ...
  const chart=page.getByTestId('punching-demand-chart');await expect(chart).toBeVisible();await expect(lab).toContainText('τ média');await expect(lab).toContainText('|τ| máximo');await expect(lab).toContainText('MPa');
  ...
  await lab.locator('[data-punch-Mx]').fill('80');await lab.locator('[data-punch-My]').fill('45');await page.getByTestId('run-punching-demand').click();const avg2=parseFloat((await kpis.nth(1).textContent())||'0'),max2=parseFloat((await kpis.nth(2).textContent())||'0');expect(max2).toBeGreaterThan(avg2);
