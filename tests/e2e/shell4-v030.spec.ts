...
test('shell4 contour supports nodal, Gauss and center recovery and exports vector SVG',async({page},testInfo)=>{
  ...
  await field.selectOption('Ux');await expect(canvas).toHaveAttribute('data-force-mode','Ux');await expect(canvas).toHaveAttribute('data-contour-geometry','deformed');await expect(status).toContainText('Ux');await expect(status).toContainText('m');
  await expect.poll(async()=>Number(await canvas.getAttribute('data-contour-min'))).not.toBeNaN();await expect.poll(async()=>Number(await canvas.getAttribute('data-contour-max'))).not.toBeNaN();
  const quickField=page.getByLabel('Campo visual rápido');await expect(quickField.locator('option[value="Umag"]')).toHaveText('Laje |u| total');await quickField.selectOption('Umag');await expect(canvas).toHaveAttribute('data-force-mode','Umag');await expect(status).toContainText('Umag');await expect(status).toContainText('m');await expect.poll(async()=>Number(await canvas.getAttribute('data-contour-min'))).toBeGreaterThanOrEqual(0);await expect.poll(async()=>Number(await canvas.getAttribute('data-contour-max'))).toBeGreaterThan(0);
  const legend=page.getByTestId('engineering-result-legend');await expect(legend).toHaveAttribute('data-result-field','Umag');await expect(legend).toHaveAttribute('data-result-unit','mm');await expect(legend.locator('.eng-legend-ticks span')).toHaveCount(9);await expect(legend.locator('.eng-gradient')).toHaveCSS('height',/\d+px/);await expect.poll(async()=>Number(await legend.locator('.eng-legend-ticks span').first().textContent())).toBeGreaterThan(0);await expect(legend.locator('.eng-legend-ticks span').last()).toHaveText(/-/);
  const finalMax=Number(await canvas.getAttribute('data-result-map-max')),finalGradient=await legend.locator('.eng-gradient').evaluate(element=>getComputedStyle(element).backgroundImage),animate=page.getByTestId('engineering-view-animation');expect(finalMax).toBeGreaterThan(0);await expect(canvas).toHaveAttribute('data-result-map-phase','1.000');await expect(canvas).toHaveAttribute('data-result-map-intensity','1.000');await animate.click();await expect(canvas).toHaveAttribute('data-result-map-live','true');
  await expect.poll(async()=>page.evaluate(({finalMax,finalGradient})=>{const canvas=document.querySelector<HTMLElement>('[data-testid="spatial-canvas-3d"]'),legend=document.querySelector<HTMLElement>('[data-testid="engineering-result-legend"]'),top=legend?.querySelector<HTMLElement>('.eng-legend-ticks span'),gradient=legend?.querySelector<HTMLElement>('.eng-gradient'),phase=Number(canvas?.dataset.resultMapPhase),intensity=Number(canvas?.dataset.resultMapIntensity),currentMax=Number(canvas?.dataset.resultMapMax),legendIntensity=Number(legend?.dataset.resultIntensity),legendMaxMm=Number(top?.textContent);return{canvasLive:canvas?.dataset.resultMapLive==='true',phaseMoving:phase<.9,intensityMoving:intensity<.9,rangeMoving:currentMax<finalMax*.9,legendLive:legend?.dataset.resultLive==='true',legendMoving:legendIntensity<.95,synchronized:Math.abs(intensity-legendIntensity)<.2,ticksMoving:legendMaxMm<finalMax*1000*.95,ticksSynchronized:Math.abs(legendMaxMm-currentMax*1000)<Math.max(.005,finalMax*200),gradientMoving:getComputedStyle(gradient!).backgroundImage!==finalGradient,labelLive:legend?.textContent?.includes('Animação')===true}},{finalMax,finalGradient})).toEqual({canvasLive:true,phaseMoving:true,intensityMoving:true,rangeMoving:true,legendLive:true,legendMoving:true,synchronized:true,ticksMoving:true,ticksSynchronized:true,gradientMoving:true,labelLive:true});
  ...
  await field.selectOption('Mx');await expect(status).toContainText('kN·m/m');
