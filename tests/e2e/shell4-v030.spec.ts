...
test('shell4 contour supports nodal, Gauss and center recovery and exports vector SVG',async({page},testInfo)=>{
  ...
  await expect.poll(async()=>page.evaluate(({finalMax,finalGradient})=>{const canvas=document.querySelector<HTMLElement>('[data-testid="spatial-canvas-3d"]'),legend=document.querySelector<HTMLElement>('[data-testid="engineering-result-legend"]'),top=legend?.querySelector<HTMLElement>('.eng-legend-ticks span'),gradient=legend?.querySelector<HTMLElement>('.eng-gradient'),phase=Number(canvas?.dataset.resultMapPhase),intensity=Number(canvas?.dataset.resultMapIntensity),currentMax=Number(canvas?.dataset.resultMapMax),legendPhase=Number(legend?.dataset.resultPhase),legendIntensity=Number(legend?.dataset.resultIntensity),legendMaxMm=Number(top?.textContent);return canvas?.dataset.resultMapLive==='true'&&legend?.dataset.resultLive==='true'&&phase>.08&&phase<.82&&intensity>.08&&intensity<.82&&currentMax>0&&currentMax<finalMax*.85&&Math.abs(phase-legendPhase)<.03&&Math.abs(intensity-legendIntensity)<.03&&Math.abs(legendMaxMm-currentMax*1000)<Math.max(.002,currentMax*1000*.02)&&getComputedStyle(gradient!).backgroundImage!==finalGradient&&legend?.textContent?.includes('Animação')===true},{finalMax,finalGradient})).toBe(true);
