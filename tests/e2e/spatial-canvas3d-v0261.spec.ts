...
test('v0.26.1/v0.30.2 renders spatial Canvas 3D with skeleton, transparent, true sections and realistic modes',async({page})=>{
  await loadProject(page);
  const canvas=page.getByTestId('spatial-canvas-3d'),mode=page.getByTestId('spatial3d-display-mode'),vector=page.getByTestId('spatial3d-vector-scene');await expect(canvas).toBeVisible();await expect(canvas).toHaveAttribute('data-view','iso');await expect(canvas).toHaveAttribute('data-display-mode','transparent');await expect(vector.locator('[data-scientific-true-sections="true"] polygon').first()).toBeVisible();
  await mode.selectOption('skeleton');await expect(canvas).toHaveAttribute('data-display-mode','skeleton');await expect(vector.locator('[data-scientific-true-sections="true"]')).toHaveCount(0);
  await mode.selectOption('trueSections');await expect(canvas).toHaveAttribute('data-display-mode','trueSections');await expect(vector.locator('[data-scientific-true-sections="true"] polygon').first()).toBeVisible();
  await mode.selectOption('realistic');await expect(canvas).toHaveAttribute('data-display-mode','realistic');await expect(vector.locator('[data-scientific-true-sections="true"] polygon').first()).toBeVisible();
