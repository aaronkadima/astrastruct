...
test('Model Lab opens hydrostatic rectangular Shell4 tank example',async({page},testInfo)=>{
  ...
  await page.goto('./');await page.evaluate(()=>window.dispatchEvent(new CustomEvent('astrastruct:model-lab-open')));const open=page.getByTestId('open-shell-water-tank-example');await expect(open).toBeVisible();await Promise.all([page.waitForEvent('framenavigated'),open.click()]);
