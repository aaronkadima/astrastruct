...
const severeKappa=1.25*p05.epscu/(section.geometry.bounds.zMax||.25),severe=sectionTx.response({epsilon0:0,kappaY:severeKappa,kappaZ:0});assert.ok(severe.crackedFibers>0);assert.ok(severe.crushedFibers>0);assert.ok(severe.yieldedBars>0);assert.equal(symmetric(severe.tangent),true);assert.notDeepEqual(sectionTx.trialHistory(),committedSection);sectionTx.rollback();assert.deepEqual(sectionTx.trialHistory(),committedSection);
