...
import {
  ...
  IFC_MATERIAL_MAPPING_CONTRACT,IFC_MATERIAL_MAPPING_VERSION,
  IFC_STEP_OWNER_CONTRACT,IFC_STEP_OWNER_VERSION,
...
assert.equal(pkg.version,'0.45.0','package.json deve declarar v0.45.0');
assert.equal(PRODUCT_VERSION,'0.45.0','PRODUCT_VERSION deve coincidir com package.json');
assert.equal(PROJECT_SCHEMA_VERSION,2,'v0.45 não altera o schema persistido');
assert.equal(RESULT_CONTRACT_VERSION,'1.0','v0.45 não altera structural-result/v1');
...
assert.equal(IFC_STEP_OWNER_CONTRACT,'ifc-step-owner/v1');
assert.equal(IFC_STEP_OWNER_VERSION,'0.45.0-exp');
assert.equal(IFC_STEP_WRITER_CONTRACT,'ifc-step-writer/v1');
assert.equal(IFC_STEP_WRITER_VERSION,'0.45.0-exp');
...
assert.equal(IFC_EXCHANGE_VERSION,'0.45.0-exp');
...
assert.ok(!pkg.scripts.test.includes('tests/release-gate-v044-smoke.mjs'),'gate histórico v0.44 não deve bloquear a versão corrente');
assert.ok(!pkg.scripts.test.includes('run-smoke.mjs'),'descoberta automática continua proibida no gate principal');

const docs=await readFile(new URL('../docs/ifc-interoperability-v045.md',import.meta.url),'utf8');
for(const phrase of [
  'IFC4X3_ADD2',
  'ISO 16739-1:2024',
  'IfcOpenShell 0.8.5',
...
])assert.ok(docs.includes(phrase),`documentação v0.45 incompleta: ${phrase}`);

console.log('AstraStruct v0.45 release gate: IFC4X3 contracts, deterministic STEP round-trip, persistent application exchange, external validation and publication boundary coherent.');
