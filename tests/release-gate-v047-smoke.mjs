...
assert.equal(pkg.version,'0.47.0','package.json deve declarar v0.47.0');
assert.equal(PRODUCT_VERSION,'0.47.0','PRODUCT_VERSION deve coincidir com package.json');
...
const mandatory=[
  'tests/release-gate-v047-smoke.mjs',
...
assert.ok(!pkg.scripts.test.includes('tests/development-gate-v047-smoke.mjs'),'development gate não deve permanecer ativo após o fechamento');
assert.ok(!pkg.scripts.test.includes('tests/release-gate-v046-smoke.mjs'),'release gate histórico v0.46 não deve bloquear a versão corrente');
...
for(const phrase of [
  'Pset_MaterialSteel','YieldStress','UltimateStress','Pset_MaterialConcrete','CompressiveStrength','fctm','IFCPRESSUREMEASURE',
  'IfcOpenShell','desktop, Android e tablet','PRODUCT_VERSION = 0.47.0','Nenhuma promoção para `main`'
])assert.ok(docs.includes(phrase),`documentação v0.47 incompleta: ${phrase}`);

console.log('AstraStruct v0.47 release gate: standard IFC steel/concrete strength round-trip, conservative typing, external validation and publication boundary coherent.');
