...
const modal=solveModal2D(project,{modes:3,massFormulation:'consistent'});assert.equal(modal.solverVersion,'0.25.0-exp');
...
assert.equal(srss.solverVersion,'0.25.0-exp');assert(srss.combined.peakTranslationalDisplacement>0);assert(Math.abs(srss.combined.peakTranslationalDisplacement-cqc.combined.peakTranslationalDisplacement)<1e-12,'one-mode SRSS and CQC must coincide');assert.equal(srss.modalContributions.length,1);
console.log('v0.25 seismic base-motion / response-spectrum smoke: OK',{pgaG:sp.pgaG,peakTH:th.peakResponse.absDisplacement,peakSpectrum:srss.combined.peakTranslationalDisplacement});
