...
export function moveSheetEntity(sheet,entityId,dx=0,dy=0){const e=sheet.entities.find(x=>String(x.id)===String(entityId));if(!e)throw new Error('DrawingSheet: entidade não encontrada.');const patch={x:e.x+finite('dx',dx),y:e.y+finite('dy',dy)};if(e.kind==='line'){patch.x2=e.x2+Number(dx);patch.y2=e.y2+Number(dy)}if(Array.isArray(e.points))patch.points=e.points.map(p=>({x:p.x+Number(dx),y:p.y+Number(dy)}));return updateSheetEntity(sheet,entityId,patch);}
