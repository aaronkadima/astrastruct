...
export function normalizeFoundationItem(item={},project={}){
  ...
  return{id:text(item.id||item.nodeId||'F-UNASSIGNED'),label:text(item.label,item.id||'Fundação'),nodeId:item.nodeId==null?null:text(item.nodeId),x:finite(item.x)??finite(node?.x)??0,y:finite(item.y)??finite(node?.y)??0,type:text(item.type,'footing'),geometry:{B:finite(item.geometry?.B),L:finite(item.geometry?.L),h:finite(item.geometry?.h),b0:finite(item.geometry?.b0),d:finite(item.geometry?.d),bw:finite(item.geometry?.bw)},demand:{N:finite(item.demand?.N),Mx:finite(item.demand?.Mx)??0,My:finite(item.demand?.My)??0,Hx:finite(item.demand?.Hx)??0,Hy:finite(item.demand?.Hy)??0,VuPunching:finite(item.demand?.VuPunching),VuOneWay:finite(item.demand?.VuOneWay),Mu:finite(item.demand?.Mu),stabilizingMoment:finite(item.demand?.stabilizingMoment),overturningMoment:finite(item.demand?.overturningMoment)},soil:{qDesign:finite(item.soil?.qDesign)},materials:{fc:finite(item.materials?.fc),fy:finite(item.materials?.fy),lambda:finite(item.materials?.lambda)??1},reinforcement:{As:finite(item.reinforcement?.As)},enabledChecks:{bearing:true,sliding:true,overturning:true,punching:true,oneWayShear:true,flexure:true,...(item.enabledChecks||{})},metadata:clone(item.metadata||{})};
}
