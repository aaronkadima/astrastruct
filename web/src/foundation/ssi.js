...
export const FOUNDATION_SSI_VERSION='0.53.4-exp';
...
export const FOUNDATION_SSI_SUPPORTED_ANALYSES=Object.freeze(['linear','corotational','pdelta','modal']);
...
function resultNature(result){if(!result)return'none';const kind=text(result.analysisType||result.type).toLowerCase();return kind.includes('modal')||kind.includes('buckling')?'eigen-shape':'physical'}
...
export function withFoundationSSI(project={},configInput={}){
  ...
  if(config.enabled){
    const analysisType=text(p?.settings?.analysisType,'linear');if(!FOUNDATION_SSI_SUPPORTED_ANALYSES.includes(analysisType))throw new Error(`SSI v0.53.4: análise '${analysisType}' ainda não usa o contrato espacial de molas. Use análise linear, P-Delta, modal ou co-rotacional 3D.`);
...
export function foundationSSIResults(project={},resultInput=null){
  const model=foundationSSIModelFromProject(project),result=resultInput||project.results||project.lastResult||project.analysisResults||null,nature=resultNature(result);
  const items=model.items.map(item=>{const physical=nature==='physical',d=physical?displacementFor(result,item.nodeId):null,f=physical?(springForceFor(result,item.foundationId,item.nodeId)||computedSpringForce(item,d)):null,contact=shallowContactGeometry(item.reviewItem),q=f&&contact.area?Number(f.fz||0)/contact.area:null,piles=Array.isArray(item.reviewItem?.piles)?item.reviewItem.piles:[],state=!item.active?'DISABLED':item.state!=='READY'?item.state:!result?'UNSOLVED':nature==='eigen-shape'?'MODE_ONLY':d&&f?'READY':'PENDING';return{foundationId:item.foundationId,nodeId:item.nodeId,label:item.label,state,stiffness:clone(item.stiffness),source:item.source,displacement:d?{ux:Number(d.ux)||0,uy:Number(d.uy)||0,uz:Number(d.uz)||0,rx:Number(d.rx)||0,ry:Number(d.ry)||0,rz:Number(d.rz)||0}:null,springReaction:f?{fx:Number(f.fx)||0,fy:Number(f.fy)||0,fz:Number(f.fz)||0,mx:Number(f.mx)||0,my:Number(f.my)||0,mz:Number(f.mz)||0}:null,contactAreaM2:contact.area,averageContactPressureKPa:q,pileReactionState:piles.length?'PENDING_DISTRIBUTION_MODEL':'NOT_APPLICABLE',notes:[nature==='eigen-shape'?'Análise modal/flambagem fornece forma própria normalizada; não é interpretada como recalque ou reação física.':null,contact.deep?'Pressão média de contato pelo bloco não calculada para fundação profunda.':contact.area?'q médio = Rz/(B·L); não representa distribuição de tensões.':'B/L explícitos ausentes: pressão média não calculada.',piles.length?'Reações por estaca exigem modelo de distribuição/rigidez explícito; divisão igualitária não é inferida.':null].filter(Boolean)}});
  return{contract:FOUNDATION_SSI_RESULT_CONTRACT,version:FOUNDATION_SSI_VERSION,solverResultAvailable:!!result,items,summary:{ready:items.filter(x=>x.state==='READY').length,unsolved:items.filter(x=>x.state==='UNSOLVED').length,modeOnly:items.filter(x=>x.state==='MODE_ONLY').length,pending:items.filter(x=>['PENDING','INVALID'].includes(x.state)).length,disabled:items.filter(x=>x.state==='DISABLED').length},governance:{reactionFormula:'R=-K*u',averageContactPressureFormula:'q=Rz/(B*L)',inferredSoilStiffness:false,inferredPileDistribution:false,resultNature:nature,eigenShapesAreNotPhysicalSettlements:true}};
}
