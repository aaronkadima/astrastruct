...
export function newtonSolve({initial,residual,tangent,strategy='full',tolerances={},linearSolver={},modifiedRefresh=0,lineSearch={}}){
  ...
  for(let iteration=1;iteration<=policy.maxIterations;iteration++){
    const refresh=strategy!=='modified'||!cachedTangent||modifiedRefresh>0&&((iteration-1)%modifiedRefresh===0);
    ...
    if(strategy==='line-search'){
      const reduction=Number(lineSearch.reduction)||0.5,minStep=Number(lineSearch.minStep)||1/128,armijo=Number(lineSearch.armijo)||1e-4,base=norm2(r);let accepted=false;
      ...
      if(!accepted){x=x.map((v,i)=>v+step*dx[i]);r=asVector(residual(x),n,'Newton residual')}
...
export function newtonStrategies(){return['full','modified','line-search']}
