const DIMS={
  ...
  stress:[1,-2,0,0],moment:[1,1,0,0],stiffness:[1,-1,0,0],lineLoad:[1,-1,0,0],area:[0,2,0,0],inertia:[0,4,0,0],velocity:[0,1,-1,0],acceleration:[0,1,-2,0]
};
const UNITS=new Map();
...
define('N/m','lineLoad',1);define('kN/m','lineLoad',1e3);define('N/mm','lineLoad',1e3);
define('N/m','stiffness',1);define('kN/m','stiffness',1e3);define('N/mm','stiffness',1e3);
...
export function toBaseUnit(value,unit){const u=unitOf(unit);return{value:Number(value)*u.factor,dimension:[...u.dimension],dimensionName:u.dimensionName}}
...
export function listUnits(dimensionName=null){return[...UNITS.values()].filter(u=>!dimensionName||u.dimensionName===dimensionName).map(u=>({...u,dimension:[...u.dimension]}))}
