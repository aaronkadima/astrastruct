...
function maxDisplacement(result){
  if(resultNature(result)!=='physical')return{magnitudeMm:null,nodeId:null};let best=null;
...
export function buildCombinationEnvelopeExplorer(project={},solvedEntries=[]){
  const inventory=buildingLevelInventory(project),descriptors=combinationDescriptors(project),descById=new Map(descriptors.map(d=>[d.id,d])),entries=(solvedEntries||[]).map((entry,i)=>{
    const id=text(entry?.combinationId||entry?.id||entry?.result?.scenario?.id,`comb-${i+1}`),descriptor=descById.get(id)||{id,name:text(entry?.name,id),category:'UNKNOWN',categorySource:'unknown',terms:[]},result=entry?.result||null,error=entry?.error?text(entry.error):null,nature=result?resultNature(result):null,disp=maxDisplacement(result),drifts=result?storyDrifts(project,inventory,result):[];
  ...
  return{contract:COMBINATION_ENVELOPE_CONTRACT,version:COMBINATION_ENVELOPE_VERSION,combinations:entries,descriptors,elementEnvelopes:[...envByElement.values()].sort((a,b)=>a.elementId.localeCompare(b.elementId)),storyDriftEnvelopes:[...driftByLevel.values()].sort((a,b)=>String(a.levelId).localeCompare(String(b.levelId))),displacementEnvelope,summary:{defined:descriptors.length,solved:entries.length,ready:ready.length,error:entries.filter(e=>e.status==='ERROR').length,pending:Math.max(0,descriptors.length-entries.length)+entries.filter(e=>e.status==='PENDING').length,elu:entries.filter(e=>e.category==='ELU').length,els:entries.filter(e=>e.category==='ELS').length,unknown:entries.filter(e=>e.category==='UNKNOWN').length,elements:envByElement.size,stories:driftByLevel.size},governance:{resultsOnly:true,noSyntheticResponses:true,noNormativeDriftPass:true,unknownCombinationCategoryNeverPromoted:true,eigenShapesExcludedFromPhysicalEnvelopes:true}};
}
