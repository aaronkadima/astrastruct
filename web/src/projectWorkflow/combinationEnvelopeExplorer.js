...
export const COMBINATION_ENVELOPE_VERSION='0.53.10-exp';
...
export function buildCombinationEnvelopeExplorer(project={},solvedEntries=[]){
  ...
  const elementById=new Map((project.elements||[]).map(e=>[String(e.id),e])),nodeById=new Map((project.nodes||[]).map(n=>[String(n.id),n])),envByElement=new Map(),nodeDispById=new Map();
