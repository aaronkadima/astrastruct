...
export function runMonteCarloReliability(project,config={},solver=solve){
  ...
  for(let i=0;i<sampleCount;i++){
    const values=config.correlationMatrix?sampleCorrelatedNormalValues(variables,config.correlationMatrix,rng).values:variables.map(variable=>sampleRandomVariable(variable,rng));
    const sampled=variables.map((variable,j)=>({id:variable.id,target:variable.target,value:values[j]})),candidate=applyTargetValues(project,sampled),result=solver(candidate,config.scenarioId),state=evaluateLimitState(result,config.limitState);if(state.failure)failures++;const k=i+1,delta=state.demand-mean;mean+=delta/k;m2+=delta*(state.demand-mean);if(config.storeSamples)stored.push({index:i,variables:Object.fromEntries(sampled.map(x=>[x.id,x.value])),demand:state.demand,g:state.g,failure:state.failure});
  }
  const pf=failures/sampleCount,interval95=wilson(failures,sampleCount),beta=pf>0&&pf<1?-inverseNormalCdf(pf):null;
  return{contract:RELIABILITY_CONTRACT,version:RELIABILITY_VERSION,method:'monte-carlo',seed,sampleCount,failures,pf,beta,interval95,demand:{mean,standardDeviation:sampleCount>1?Math.sqrt(m2/(sampleCount-1)):0},variables:variables.map(v=>({id:v.id,distribution:v.distribution||'deterministic',mean:v.mean??v.value,standardDeviation:v.standardDeviation??v.sd??0,target:v.target})),limitState:config.limitState,correlationMatrix:config.correlationMatrix??null,samples:config.storeSamples?stored:undefined};
}
