...
function contactKinematics(dx,dy,b){
  ...
  return{
    relativeMagnitude:Math.hypot(dx,dy),
    along,
    slotCoordinate,
...
function responseAt(q,bolts){
  ...
  for(const b of bolts){
    ...
    items.push({
      ...b,
      hole:{type:b.slotHalfLength>EPS?'slotted':'circular',gap:b.gap,slotHalfLength:b.slotHalfLength,slotLength:2*b.slotHalfLength,slotAngle:b.slotAngle},
      relativeDisplacement:{x:dx,y:dy,magnitude:kin.relativeMagnitude},
      slotCoordinate:kin.slotCoordinate,
      closestPoint:kin.closestPoint,
      contactNormal:kin.contactNormal,
      normalDistance:kin.normalDistance,
      penetration:kin.penetration,
      force:{fx,fy,magnitude:law.force,moment:mz},
      state:law.state,
      tangent:law.tangent,
      utilization:b.shearCapacity>EPS?law.force/b.shearCapacity:null
    });
  }
...
function jacobian(q,bolts,hBase=1e-7){
  const J=mat3();
  for(let j=0;j<3;j++){
    const h=Math.max(1e-10,hBase*Math.max(1,Math.abs(q[j]))),qp=[...q],qm=[...q];
    qp[j]+=h;qm[j]-=h;
...
function predictor(P,bolts){
  ...
  for(const b of bolts){
    const dx=q[0]-q[2]*b.y,dy=q[1]+q[2]*b.x,r=Math.hypot(dx,dy),outerReach=b.gap+b.slotHalfLength;
    if(outerReach>0&&r>EPS)scale=Math.max(scale,outerReach/r*1.05);
  }
  if(scale>1)q=q.map(v=>v*scale);
  return q;
}

export function generateSlottedContactBoltGroup({
  nx=2,ny=2,spacingX=.20,spacingY=.15,bearingStiffness=250000,gap=.001,
  slotLength=.012,slotAngle=0,shearCapacity=80,yieldForce=0,postYieldRatio=.02
}={}){
  const half=.5*Math.max(0,finite(slotLength,.012)),angle=finite(slotAngle);
  return generateRectangularBoltGroup({nx,ny,spacingX,spacingY,kx:1,ky:1,shearCapacity}).map(b=>({
    id:b.id,x:b.x,y:b.y,k:Math.max(EPS,finite(bearingStiffness,250000)),gap:Math.max(0,finite(gap,.001)),
    slotHalfLength:half,slotAngle:angle,shearCapacity:Math.max(0,finite(shearCapacity,80)),yieldForce:Math.max(0,finite(yieldForce)),
    postYieldRatio:Math.max(0,finite(postYieldRatio,.02))
  }));
}
...
export function solveBoltGroupSlottedClearanceContact({bolts=[],loads={},steps=12,maxIterations=60,tolerance=1e-9}={}){
  ...
  for(let step=1;step<=nSteps;step++){
    const lambda=step/nSteps,P=lv.P.map(v=>v*lambda);
    if(step===1)q=predictor(P,bs);
    let converged=false,iterations=0,norm=Infinity;
    for(iterations=1;iterations<=maxIterations;iterations++){
      const state=responseAt(q,bs),R=P.map((v,i)=>v-state.transferred[i]);
      norm=residualNorm(R,P,L);
      if(norm<=tol){last=state;converged=true;break}
      const J=jacobian(q,bs),scale=Math.max(EPS,...J.flat().map(v=>Math.abs(v))),reg=scale*1e-10;
      for(let i=0;i<3;i++)J[i][i]+=reg;
      const dq=solve3(J,R);let accepted=false,alpha=1;
      for(let ls=0;ls<14;ls++){
        const trial=q.map((v,i)=>v+alpha*dq[i]),st=responseAt(trial,bs),Rt=P.map((v,i)=>v-st.transferred[i]),nt=residualNorm(Rt,P,L);
        if(nt<norm||alpha<1e-4){q=trial;accepted=true;break}
        alpha*=.5;
      }
      if(!accepted)q=q.map((v,i)=>v+dq[i]*1e-4);
    }
    if(!converged){
      const state=responseAt(q,bs),R=P.map((v,i)=>v-state.transferred[i]);norm=residualNorm(R,P,L);
      if(norm>Math.max(tol*10,1e-7))throw new Error(`Contato em furo oblongo: Newton não convergiu no passo ${step}/${nSteps} (resíduo ${norm.toExponential(3)}).`);
      last=state;
    }
  ...
  return{
    type:'bolt-group-slotted-clearance-contact-2d',solverVersion:'0.30.1',bolts:final.items,
    plateDisplacement:{ux:q[0],uy:q[1],theta:q[2]},loads:{...lv},
    equilibrium:{transferred:{fx:final.transferred[0],fy:final.transferred[1],mz:final.transferred[2]},residual:{fx:R[0],fy:R[1],mz:R[2]},residualNorm:rn},
    history,maxBolt,maxUtilization,activeBolts:final.items.filter(b=>b.state!=='gap').length,
    assumptions:[
      'placa rígida no plano',
      'furo circular ou oblongo idealizado como cápsula (trecho reto + extremidades semicirculares)',
      'movimento livre sem força dentro da folga e do comprimento do rasgo',
      'contato unilateral normal ao contorno após fechamento da folga',
      'lei normal elástica ou bilinear de bearing definida pelo usuário',
      'Newton-Raphson incremental com tangente numérica',
      'sem atrito, pré-tensão, prying, flexibilidade explícita da chapa ou resistência normativa automática'
    ]
  };
}
