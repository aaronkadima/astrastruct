...
/**
 * v0.18 spherical arc-length / Riks continuation with tangent-stability tracking.
...
export function solveFrameCorotationalArcLength2D(project,scenarioId,options={}){
  ...
  let lambda=0,radius=initialRadius,last=initial,previous=null,firstYield=null,pendingBranchSwitch=null;const history=[],yieldedKeys=new Set(),stabilityEvents=[],branchSwitches=[];
  ...
  const base={type:'frame2d-corotational-arc-length-experimental',solverVersion:'0.18.0-exp',scenario:resolved.scenario,dofs:prepared.nd,activeDofs:prepared.free.length,displacements,initialDisplacements,totalDisplacements,initialGeometry,reactions,elementForces,imperfection:imperfectionMeta,arcLength,nonlinear:{formulation:'2D co-rotational Euler-Bernoulli',controlMode:'arc-length',steps,maxIterations,tolerance,absoluteTolerance,lineSearch,finalLoadFactor:lambda,loadModel:'scaled reference load pattern + thermal initial strain/curvature + follower end forces',followerLoads:{count:followerCount,externalTangent:'consistent',supported:'element end 2 concentrated force'},endConnections:{flexibleEndCount,method:'internal end rotations + Schur condensation',customResolver:typeof options.connectionResolver==='function',experimental:true},imperfection:imperfectionMeta,arcLength,history,converged:true}};
  return{...base,elementResponses:buildCorotationalResponses(p,base,41)};
}
