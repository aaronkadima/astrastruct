...
import { analyzeTangentSpectrum, detectMultimodeTransitions, applyReferenceEigenvalueRatios } from './stabilityMultimode2d.js';
import { distributedSteelFiberBasicState, normalizeDistributedPlasticityConfig, validateDistributedPlasticityModel } from './distributedPlasticity2d.js';
...
/** v0.20 displacement-based distributed steel fiber element. */
export function corotationalDistributedPlasticityElementState({X1,Y1,X2,Y2,qGlobal,E,A,I,initialBasic=[0,0,0],section,material,distributedPlasticity={},committedHistory=null}){
  ...
  const c=dx/l,s=dy/l,alpha=Math.atan2(dy,dx),dAlpha=wrapAngle(alpha-alpha0),basic=[l-L0,q[2]-dAlpha,q[5]-dAlpha],initial=initialBasic.map(Number),elasticBasic=subVector(basic,initial);
  const distributed=distributedSteelFiberBasicState({elasticBasic,L0,section,material,config:distributedPlasticity,committedHistory}),basicForces=distributed.basicForces,kb=distributed.basicTangent,[N,M1,M2]=basicForces;
...
export function solveFrameCorotational2D(project,scenarioId,options={}){
  ...
  const base={type:'frame2d-corotational-experimental',solverVersion:hasDistributedPlasticity(prepared)?'0.20.0-exp':'0.13.6-exp',scenario:resolved.scenario,dofs:prepared.nd,activeDofs:prepared.free.length,displacements,initialDisplacements,totalDisplacements,initialGeometry,reactions,elementForces,imperfection:imperfectionMeta,distributedPlasticity:distributedPlasticityResult(last.states),nonlinear:{formulation:'2D co-rotational Euler-Bernoulli',steps,maxIterations,tolerance,lineSearch,loadModel:'reference-dead mechanical loads + thermal initial strain/curvature + follower end forces',followerLoads:{count:followerCount,externalTangent:'consistent',supported:'element end 2 concentrated force'},endConnections:{flexibleEndCount,method:'internal end rotations + Schur condensation',customResolver:typeof options.connectionResolver==='function',experimental:true},imperfection:imperfectionMeta,history,converged:true}};
...
function distributedPlasticityResult(states=[]){
  const records=[];
  for(const entry of states||[]){
...
export function solveFrameCorotationalDisplacementControl2D(project,scenarioId,options={}){
  ...
  for(let step=1;step<=steps;step++){
    const target=targets[step-1],controlScale=Math.max(1e-8,...targets.map(Math.abs)),controlTolAbs=controlTolerance*controlScale;let converged=false,iteration=0;
    for(iteration=1;iteration<=maxIterations;iteration++){
      const current=residualAt(prepared,u,lambda,analysisOptions),scale=residualScale(prepared,current,lambda),controlError=target-u[control.index];
      ...
      const Kff=prepared.free.map(i=>prepared.free.map(j=>current.K[i][j])),rf=prepared.free.map(i=>current.residual[i]),dRdl=residualLoadDerivative(prepared,u,lambda,analysisOptions),pf=prepared.free.map(i=>dRdl[i]),nf=prepared.free.length,augmented=Kff.map((row,i)=>[...row,-pf[i]]),constraint=Array(nf+1).fill(0);constraint[control.freePosition]=1;augmented.push(constraint);let correction;
      ...
      if(lineSearch){
        for(let ls=0;ls<8;ls++){
          const trial=[...u];prepared.free.forEach((d,k)=>{trial[d]+=eta*du[k]});const trialLambda=lambda+eta*dLambda,candidate=residualAt(prepared,trial,trialLambda,analysisOptions),trialError=target-trial[control.index],merit=displacementControlMerit(prepared,candidate,trialError,trialLambda,control.targetDisplacement);
          if(merit<currentMerit||eta<=1/128){next={trial,trialLambda,candidate};break}eta*=.5;
        }
      }
      if(!next){const trial=[...u];prepared.free.forEach((d,k)=>{trial[d]+=du[k]});const trialLambda=lambda+dLambda;next={trial,trialLambda,candidate:residualAt(prepared,trial,trialLambda,analysisOptions)}}
    ...
    if(!converged){const current=residualAt(prepared,u,lambda,analysisOptions),scale=residualScale(prepared,current,lambda),controlError=target-u[control.index];if(current.norm<=Math.max(tolerance*scale,absoluteTolerance)&&Math.abs(controlError)<=controlTolAbs){converged=true;last=current}}
    if(!converged){const diagnostic=residualAt(prepared,u,lambda,analysisOptions),controlError=target-u[control.index],scale=residualScale(prepared,diagnostic,lambda);throw new Error(`Pushover não convergiu no passo ${step}/${steps} em ${maxIterations} iterações: lambda=${lambda}, u=${u[control.index]}, alvo=${target}, erroControle=${controlError}, |R|=${diagnostic.norm}, escalaR=${scale}.`)}
  ...
  last=residualAt(prepared,u,lambda,analysisOptions);
  const reactions=prepared.nodes.map((n,i)=>({nodeId:n.id,fx:last.fint[3*i]-last.external[3*i],fy:last.fint[3*i+1]-last.external[3*i+1],mz:last.fint[3*i+2]-last.external[3*i+2]})),displacements=prepared.nodes.map((n,i)=>({nodeId:n.id,ux:u[3*i],uy:u[3*i+1],rz:u[3*i+2]}));
  const initialDisplacements=prepared.imperfection?prepared.nominalNodes.map((n,i)=>({nodeId:n.id,ux:prepared.imperfection.vector[3*i],uy:prepared.imperfection.vector[3*i+1],rz:prepared.imperfection.vector[3*i+2]})):null;
  const totalDisplacements=prepared.imperfection?prepared.nominalNodes.map((n,i)=>({nodeId:n.id,ux:prepared.imperfection.vector[3*i]+u[3*i],uy:prepared.imperfection.vector[3*i+1]+u[3*i+1],rz:prepared.imperfection.vector[3*i+2]+u[3*i+2]})):null;
  const initialGeometry=prepared.imperfection?prepared.nominalNodes.map((n,i)=>({nodeId:n.id,nominal:{x:Number(n.x),y:Number(n.y)},reference:{x:Number(prepared.nodes[i].x),y:Number(prepared.nodes[i].y)},offset:{ux:prepared.imperfection.vector[3*i],uy:prepared.imperfection.vector[3*i+1],rz:prepared.imperfection.vector[3*i+2]}})):null;
  ...
  const curve=history.map(h=>({step:h.step,controlledDisplacement:h.controlledDisplacement,loadFactor:h.loadFactor,baseReaction:h.baseReaction,yieldedHingeCount:h.yieldedHingeCount,newlyYieldedHinges:h.newlyYieldedHinges})),peak=curve.reduce((best,row)=>!best||Math.abs(row.loadFactor)>Math.abs(best.loadFactor)?row:best,null),pushover={enabled:true,method:cyclicProtocol.enabled?'cyclic-displacement-control':'displacement-control',control,finalLoadFactor:lambda,curve,peakLoadFactor:peak?.loadFactor??lambda,peakControlledDisplacement:peak?.controlledDisplacement??u[control.index],firstYield,loadFactorDerivative:'centered finite difference of complete residual',arcLength:false,cyclicProtocol:{enabled:cyclicProtocol.enabled,targets:[...cyclicProtocol.targets],stepsPerSegment:cyclicProtocol.stepsPerSegment,totalSteps:steps,commitRollback:(cyclicMaterial||typeof analysisOptions.commitMaterialHistory==='function')?'committed after converged global step; trial states discarded on Newton/line-search rejection':'not active'},cumulativeDissipatedEnergy:cyclicMaterial?cyclicHistoryEnergy(analysisOptions.distributedPlasticityHistory):0};
  const base={type:'frame2d-corotational-displacement-control-experimental',solverVersion:cyclicMaterial?'0.21.0-exp':(hasDistributedPlasticity(prepared)?'0.20.0-exp':'0.16.0-exp'),scenario:resolved.scenario,dofs:prepared.nd,activeDofs:prepared.free.length,displacements,initialDisplacements,totalDisplacements,initialGeometry,reactions,elementForces,imperfection:imperfectionMeta,distributedPlasticity:distributedPlasticityResult(last.states),pushover,nonlinear:{formulation:'2D co-rotational Euler-Bernoulli',controlMode:cyclicProtocol.enabled?'cyclic-displacement':'displacement',steps,maxIterations,tolerance,absoluteTolerance,controlTolerance,lineSearch,finalLoadFactor:lambda,loadModel:'scaled reference load pattern + thermal initial strain/curvature + follower end forces',followerLoads:{count:followerCount,externalTangent:'consistent',supported:'element end 2 concentrated force'},endConnections:{flexibleEndCount,method:'internal end rotations + Schur condensation',customResolver:typeof options.connectionResolver==='function',experimental:true},imperfection:imperfectionMeta,pushover,history,converged:true}};
  return{...base,elementResponses:buildCorotationalResponses(p,base,41)};
}
...
/**
 * v0.19 spherical arc-length / Riks continuation with multimode tangent-stability tracking.
...
export function solveFrameCorotationalArcLength2D(project,scenarioId,options={}){
  ...
  let lambda=0,radius=initialRadius,last=initial,previous=null,firstYield=null,pendingBranchSwitch=null;const history=[],yieldedKeys=new Set(),stabilityEvents=[],branchSwitches=[],branchExplorations=[];
  ...
  for(let step=1;step<=steps;step++){
    ...
    for(let attempt=0;attempt<=maxCutbacks;attempt++){
      ...
      try{
        ...
        for(iteration=1;iteration<=maxIterations;iteration++){
          ...
          const currentMerit=branchRequest?Math.hypot(last.norm/residualScale(prepared,last,lambda),Math.abs(g)/Math.max(Math.abs(branchTarget),1e-12)):arcMerit(prepared,last,g,lambda,radius);let eta=1,next=null;
    ...
    history.push({step,loadFactor:lambda,monitoredDisplacement:u[monitor.index],baseReaction:baseReactionAt(prepared,last,monitor.dof),iterations:iteration,residualNorm:last.norm,arcConstraint:branchRequest?null:arcConstraint(stepDu,stepDl,weights,alpha,usedRadius),continuationConstraint:branchRequest?'critical-mode-seed':'arc-length',modalConstraintResidual:branchRequest?arcDot(stepDu,branchMode,weights)-branchTarget:null,arcRadius:usedRadius,arcNorm:stepNorm,loadIncrement:stepDl,cutbacks:cutbacksUsed,materialLocalIterations,yieldedHingeCount:yielded.length,yieldedHinges:yielded,newlyYieldedHinges:newlyYielded,criticalEigenvalue:stabilityState?.enabled?stabilityState.eigenvalue:null,eigenvalueRatio:stabilityState?.enabled?stabilityState.eigenvalueRatio:null,tangentAsymmetry:stabilityState?.enabled?stabilityState.tangentAsymmetry:null,nearSingular:stabilityState?.enabled?Math.abs(stabilityState.eigenvalueRatio)<=stabilityTolerance:false,stabilityEvent:newStabilityEvents[0]?{type:newStabilityEvents[0].type,criticalLoadFactor:newStabilityEvents[0].criticalLoadFactor,dominant:newStabilityEvents[0].dominant}:null,branchSwitch:branchSwitchRecord,criticalModes:(stabilityState?.modes||[]).map(m=>({id:m.id,rank:m.rank,eigenvalue:m.eigenvalue,eigenvalueRatio:m.eigenvalueRatio,macToPrevious:m.macToPrevious,clusterId:m.clusterId,clusterSize:m.clusterSize,subspaceContinuity:m.subspaceContinuity,dominant:m.dominant})),stabilityEvents:newStabilityEvents.map(e=>({type:e.type,modeId:e.modeId,clusterId:e.clusterId,criticalLoadFactor:e.criticalLoadFactor,mac:e.mac}))});
  ...
  const base={type:'frame2d-corotational-arc-length-experimental',solverVersion:hasDistributedPlasticity(prepared)?'0.20.0-exp':'0.19.0-exp',scenario:resolved.scenario,dofs:prepared.nd,activeDofs:prepared.free.length,displacements,initialDisplacements,totalDisplacements,initialGeometry,reactions,elementForces,imperfection:imperfectionMeta,distributedPlasticity:distributedPlasticityResult(last.states),arcLength,nonlinear:{formulation:'2D co-rotational Euler-Bernoulli',controlMode:'arc-length',steps,maxIterations,tolerance,absoluteTolerance,lineSearch,finalLoadFactor:lambda,loadModel:'scaled reference load pattern + thermal initial strain/curvature + follower end forces',followerLoads:{count:followerCount,externalTangent:'consistent',supported:'element end 2 concentrated force'},endConnections:{flexibleEndCount,method:'internal end rotations + Schur condensation',customResolver:typeof options.connectionResolver==='function',experimental:true},imperfection:imperfectionMeta,arcLength,history,converged:true}};
  return{...base,elementResponses:buildCorotationalResponses(p,base,41)};
}
