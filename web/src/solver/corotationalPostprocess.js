...
export function buildCorotationalResponses(project,result,samples=41){
  ...
  for(const e of project.elements||[]){
    ...
    if(!(l>1e-12&&L0>1e-12))continue;const c=dx/l,s=dy/l,basic=f.corotational?.basic||[l-L0,0,0],phi1=num(basic[1]),phi2=num(basic[2]),N=num(f.basicForces?.N,.5*(num(f.N2)-num(f.N1))),M1=num(f.basicForces?.M1,f.M1),M2=num(f.basicForces?.M2,f.M2),V=(M1+M2)/l;
    const sec=(project.sections||[]).find(x=>x.id===e.sectionId),depth=sectionDepth(sec),cY=depth/2,A=num(e.A),I=num(e.I),flexuralStressAvailable=cY>0&&I>0,stations=[];
    for(let i=0;i<count;i++){
      const xi=i/(count-1),xc=xi*l,w=hermiteTransverse(phi1,phi2,l,xi),slope=hermiteSlope(phi1,phi2,xi),xd=x1+c*xc-s*w,yd=y1+s*xc+c*w,x0=num(a.x)+xi*(num(b.x)-num(a.x)),y0=num(a.y)+xi*(num(b.y)-num(a.y)),M=-M1+V*xc,sigmaAxial=A>0?N/A/1000:null,sigmaB=flexuralStressAvailable?M*cY/I/1000:null,sigmaTop=sigmaB==null?null:sigmaAxial-sigmaB,sigmaBottom=sigmaB==null?null:sigmaAxial+sigmaB,sigmaAbs=sigmaB==null?(sigmaAxial==null?null:Math.abs(sigmaAxial)):Math.max(Math.abs(sigmaTop),Math.abs(sigmaBottom));
    ...
    responses.push({elementId:e.id,type:'frame2d-corotational',L:L0,currentLength:l,c,s,currentGeometry:true,stress:{available:flexuralStressAvailable,axialOnly:!flexuralStressAvailable,cY,unit:'MPa'},convention:{N:'tração positiva',V:'+y co-rotante positivo',M:'sagente positivo',stress:'tração positiva'},stations});
  }
  return responses;
}
