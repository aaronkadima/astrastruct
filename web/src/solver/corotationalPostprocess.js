...
export function buildCorotationalResponses(project,result,samples=41){
  ...
  for(const e of project.elements||[]){
    ...
    responses.push({elementId:e.id,type:'frame2d-corotational',L:L0,currentLength:l,c,s,currentGeometry:true,qx,qy,pointLoads,thermal,loadModel:load.mode,referenceLoad:{globalPerReferenceLength:{x:load.gx,y:load.gy},currentScale:load.scale,uniform:f.loadSummary?.uniform||{qx:0,qy:0},selfWeight:num(f.loadSummary?.selfWeight),points:f.loadSummary?.points||[],currentPoints:pointLoads},loadRecovery:{equilibriumResidual,equilibriumResidualAbs},stress:{available:flexuralStressAvailable,axialOnly:!flexuralStressAvailable,cY,unit:'MPa'},convention:{N:'tração positiva',V:'+y co-rotante positivo',M:'sagente positivo',stress:'tração positiva'},stations});
  }
  return responses;
}
