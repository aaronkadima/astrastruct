...
function currentUniformLoad(force,L0,l,c,s,loadFactor=1){
  ...
  const gx=num(global.x),gy=num(global.y),scale=L0/l,appliedScale=scale*loadFactor;
  return{qx:(c*gx+s*gy)*appliedScale,qy:(-s*gx+c*gy)*appliedScale,gx,gy,scale,appliedScale,loadFactor,mode:force.loadSummary?.mode||'reference-dead'};
}

function currentPointLoads(force,l,c,s,loadFactor=1){
  return(force.loadSummary?.points||[]).map(p=>{
    const xi=clamp(num(p.xi,.5),0,1),gx=num(p.global?.x)*loadFactor,gy=num(p.global?.y)*loadFactor;
...
export function buildCorotationalResponses(project,result,samples=41){
  ...
  for(const e of project.elements||[]){
    ...
    const c=dx/l,s=dy/l,basic=f.corotational?.basic||[l-L0,0,0],phi1=num(basic[1]),phi2=num(basic[2]),load=currentUniformLoad(f,L0,l,c,s,loadFactor),pointLoads=currentPointLoads(f,l,c,s,loadFactor),qx=load.qx,qy=load.qy,N1=num(f.N1),V1=num(f.V1),M1=num(f.M1),N2=num(f.N2),V2=num(f.V2),M2=num(f.M2),stations=[];
    ...
    responses.push({elementId:e.id,type:'frame2d-corotational',loadFactor,L:L0,nominalLength:Lnom,currentLength:l,c,s,currentGeometry:true,initialGeometry:!!result.initialDisplacements,qx,qy,pointLoads,thermal,followerEnds,nonconservative:followerEnds.length>0,loadModel:load.mode,referenceLoad:{globalPerReferenceLength:{x:load.gx,y:load.gy},currentScale:load.scale,appliedScale:load.appliedScale,loadFactor,uniform:f.loadSummary?.uniform||{qx:0,qy:0},selfWeight:num(f.loadSummary?.selfWeight),points:f.loadSummary?.points||[],currentPoints:pointLoads},loadRecovery:{equilibriumResidual,equilibriumResidualAbs},stress:{available:flexuralStressAvailable,axialOnly:!flexuralStressAvailable,cY,unit:'MPa'},convention:{N:'tração positiva',V:'+y co-rotante positivo',M:'sagente positivo',stress:'tração positiva',follower:'Px/Py constantes nos eixos locais da corda corrente',geometry:'nominal → referência imperfeita sem tensões → equilíbrio corrente'},stations});
  }
  return responses;
}
