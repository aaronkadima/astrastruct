...
export function buildRigidDiaphragmTransform3D(project,nodes,{tolerance=1e-6}={}){
  const defs=activeDefinitions(project),nd=6*nodes.length;if(!defs.length){const T=zeros(nd);for(let i=0;i<nd;i++)T[i][i]=1;return{active:false,T,retained:Array.from({length:nd},(_,i)=>i),eliminated:new Set(),diaphragmDofs:new Set(),groups:[],fullToReduced:new Map(Array.from({length:nd},(_,i)=>[i,i]))}}
  const nodeMap=new Map(nodes.map((n,i)=>[String(n.id),{node:n,index:i}])),claimed=new Set(),groups=[],eliminated=new Set(),diaphragmDofs=new Set(),tol=Math.max(1e-9,Math.abs(finite(tolerance,1e-6)));
  for(let k=0;k<defs.length;k++){
    ...
    for(const n of group){const id=String(n.id);if(claimed.has(id))throw new Error(`Nó ${id} pertence a mais de um diafragma rígido.`);claimed.add(id);const info=nodeMap.get(id);for(const d of [0,1,5])diaphragmDofs.add(6*info.index+d)}
  ...
  return{active:true,T,retained,eliminated,diaphragmDofs,groups,fullToReduced};
}

function reducePrescribed(prescribed,transform){
  const out=new Map();for(const [d,value] of prescribed||new Map()){
    if(transform.diaphragmDofs.has(d))throw new Error(`Diafragma rígido: apoio/deslocamento prescrito no DOF em plano ${d} de um nó do diafragma ainda não é suportado; remova a restrição ou exclua esse nível do diafragma.`);
...
export function solveRigidDiaphragmSystem3D(K,F,prescribed,project,nodes,options={}){
  const transform=buildRigidDiaphragmTransform3D(project,nodes,options);if(!transform.active)return{...solveConstrained(K,F,prescribed),reducedDofs:F.length,diaphragms:{active:false,count:0,groups:[],constraintForces:[]}};
  const T=transform.T,Tt=transpose(T),Kr=mm(Tt,mm(K,T)),Fr=mv(Tt,F),pr=reducePrescribed(prescribed,transform),solved=solveConstrained(Kr,Fr,pr),u=mv(T,solved.u),rawResidual=mul(K,u).map((v,i)=>v-F[i]);
  // Only physically prescribed full DOFs are reported as support reactions.
  // Residuals on free diaphragm DOFs are internal MPC forces and are exposed separately.
  const R=rawResidual.map((v,d)=>(prescribed?.has(d)?v:0)),constraintForces=[];
  for(const g of transform.groups){for(const nodeId of g.nodeIds){const i=nodes.findIndex(n=>String(n.id)===nodeId),o=6*i;constraintForces.push({diaphragmId:g.id,nodeId,role:nodeId===g.masterNodeId?'master':'slave',fx:rawResidual[o],fy:rawResidual[o+1],mz:rawResidual[o+5]})}}
  return{u,R,rawResidual,free:solved.free,constrained:solved.constrained,reducedDofs:T[0]?.length||0,diaphragms:{active:true,count:transform.groups.length,groups:transform.groups,constraintForces,formulation:'exact linear kinematic transformation; rigid XY diaphragm; no penalty stiffness',reactionConvention:'support reactions exclude internal diaphragm constraint forces'}};
}
