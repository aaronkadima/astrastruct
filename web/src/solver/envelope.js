...
export function solveEnvelope(project,scenarioIds=null){
  if(project.settings?.analysisType==='corotational')throw new Error('Envelope de cenários ainda não está habilitado para a análise geometricamente não linear co-rotacional. Resolva e inspecione cada cenário separadamente nesta versão experimental.');
  ...
  for(const result of solved){
    const ds=result.totalDisplacements||result.displacements||[];
    for(const d of ds){let env=nodeMap.get(d.nodeId);if(!env){env={nodeId:d.nodeId,ux:range(d.ux),uy:range(d.uy),rz:range(d.rz)};nodeMap.set(d.nodeId,env)}else{env.ux=extend(env.ux,d.ux);env.uy=extend(env.uy,d.uy);env.rz=extend(env.rz,d.rz)}}
  ...
  return{type:'scenario-envelope',analysisType:project.settings?.analysisType||'linear',scenarioIds:ids,scenarios:solved.map(r=>r.scenario),nodeDisplacements:[...nodeMap.values()],elementResponses:[...elementMap.values()],solverVersion:'0.13.0-exp'};
}
