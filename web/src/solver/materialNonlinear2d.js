...
function validateHinges(project,hinges){
  for(const h of hinges){
    ...
    if(!sec)throw new Error(`Rótula de fibras ${h.elementId}: seção ausente.`);
    if(String(mat.type||'').toLowerCase()!=='steel')throw new Error(`Rótula de fibras ${h.elementId}: v0.14.2 aceita apenas material do tipo steel.`);
 ...
 *
 * Strong-axis steel fiber meshes currently support rectangular, I/H and RHS
 * sections. A legacy `materialStrategy: 'secant'` mode keeps the v0.14 k=M/theta
 * update for regression/fallback. The default tangent mode is still an outer
 * material iteration, not yet a fully monolithic Newton solve with axial-material
 * coupling. It remains monotonic concentrated plasticity and does not claim cyclic
 * history, distributed plasticity, local buckling, concrete damage or bond-slip.
 */
export function solveFrameCorotationalFiberHinges2D(project,scenarioId,options={}){
  ...
  for(let outer=1;outer<=maxOuter;outer++){
    ...
    for(const h of hinges){
      ...
      const e=working.elements.find(x=>x.id===h.elementId),appliedStiffness=Number(connection.k),appliedOffsetMoment=Number(connection.offsetMoment)||0,oldK=Number(e.rotationalSprings?.[h.key]),oldOffset=Number(e.rotationalSpringMoments?.[h.key])||0,nextK=Number.isFinite(oldK)&&oldK>0?(1-relaxation)*oldK+relaxation*targetStiffness:targetStiffness,nextOffset=(1-relaxation)*oldOffset+relaxation*targetOffsetMoment;
  ...
  const worst=Math.max(0,...records.map(r=>r.relativeResidual));throw new Error(`Rótulas de fibras v0.14.2 não convergiram em ${maxOuter} iterações externas (resíduo relativo máximo=${worst.toExponential(3)}).`);
}

export const MATERIAL_NONLINEAR_VERSION='0.14.2-exp';
