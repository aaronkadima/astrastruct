...
export function solveFrameCorotationalFiberHinges2D(project,scenarioId,options={}){
  ...
  for(let outer=1;outer<=maxOuter;outer++){
    ...
    for(const h of hinges){
      ...
      records.push({elementId:h.elementId,end:h.end,key:h.key,rotation,elementMoment,constitutiveMoment,momentResidual,relativeResidual,secantStiffness,tangent:state.rotationalTangent,appliedStiffness,appliedOffsetMoment,nextStiffness:Math.max(1e-9,nextK),nextOffsetMoment:nextOffset,targetOffsetMoment,hingeLength:h.config.hingeLength,targetAxialForce,axialResidual:state.axialResidual,epsilon0:state.epsilon0,yieldedFibers:state.yieldedFibers,fiberCount:state.fiberCount,hardeningRatio:h.config.hardeningRatio});
    }
