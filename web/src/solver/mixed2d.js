...
export function solveMixed2D(project) {
  ...
  for (const l of project.loads || []) {
    ...
    F[3*i]+=l.fx||0; F[3*i+1]+=l.fy||0; F[3*i+2]+=l.mz||0;
  }

  const prescribed=new Map();
  for(const support of project.supports||[]){
    const i=map.get(support.nodeId); if(i==null)continue;
  ...
  const elementForces=cache.map(item=>{
    if(item.kind==='frame2d'){
      ...
      return {elementId:item.e.id,type:'frame2d',N1:q[0],V1:q[1],M1:q[2],N2:q[3],V2:q[4],M2:q[5],localDisplacements:ul};
    }
  ...
  return {
    type:'mixed2d',solverVersion:'0.5.0',dofs:nd,activeDofs:free.length,
