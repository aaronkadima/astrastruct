...
import { addSpatialNodalSprings } from './springs.js';
...
const projectByMap=(Q,B)=>mm(transpose(B),mm(Q,B));
...
function framePrepared(project,e,a,b){
  ...
  const kl=frame3DLocalStiffness({E,G,A,Iy,Iz,J,L:axes.L}),connection=condenseEndConnections3D(kl,Array(12).fill(0),e.releases,e.rotationalSprings),kin=endConnectionKinematicMap3D(kl,e.releases,e.rotationalSprings),T=transform12(axes.R),kg=mm(transpose(T),mm(connection.kEff,T));
  return{axes,mat,sec,E,G,A,Iy,Iz,J,kl,kElasticEff:connection.kEff,B:kin.B,T,kg};
}
...
export function assembleDynamicSystem3D(project,{massFormulation='consistent'}={}){
  ...
  const Kf=free.map(i=>free.map(j=>K[i][j])),Mf=free.map(i=>free.map(j=>M[i][j])),springCount=(project.nodeSprings||[]).length;cholesky(symmetrize(Kf),'Dinâmica modal 3D');return{nodes,elements,nd,K,M,free,Kf,Mf,massFormulation:kind,gravity:G0,connectionAware:true,shellCount,hasShell:shellCount>0,springCount};
}
...
export function solveModal3D(project,options={}){
  ...
  return{type:'dynamic-modal3d',dimension:'3d',analysisType:'modal',solverVersion:sys.hasShell?'0.30.0':'0.27.0',dofs:sys.nd,freeDofs:sys.free.length,massFormulation:sys.massFormulation,gravity:G0,densityConvention:'material.density interpreted as unit weight [kN/m³]; mass density = density/g',connectionAware:true,shellCount:sys.shellCount,springCount:sys.springCount,releaseCount,semiRigidConnectionCount,eigenIterations:eig.iterations,totalParticipatingMass:{x:Mx,y:My,z:Mz},modes,displacements:modes[0]?.displacements||[],modal:{modes,requestedModes:requested,connectionKinematics:'massless end-connection static condensation projected into stiffness and consistent/lumped member mass',foundationSprings:'spatial nodeSprings contribute to elastic K only; modal vectors are normalized eigen-shapes, not physical settlements/reactions',shellMass:sys.hasShell?'shell4 surface mass rho*t with Mindlin rotary inertia rho*t^3/12; optional drilling regularization':null}};
}
...
export function solveBuckling3D(project,scenarioId,options={}){
  ...
  for(const e of elements){const i=map.get(e.n1),j=map.get(e.n2);if(i==null||j==null)throw new Error(`Flambagem 3D: ${e.id} referencia nó inexistente.`);const prep=framePrepared(p,e,nodes[i],nodes[j]),idx=elementIdx(i,j),N=axialMap.get(e.id)||0,kglMember=frame3DLocalGeometricStiffness(N,prep.axes.L),kgl=projectByMap(kglMember,prep.B),kgg=mm(transpose(prep.T),mm(kgl,prep.T));addSub(K,prep.kg,idx);addSub(Kg,kgg,idx)}
  addSpatialNodalSprings(K,p,map,6);
  ...
  const releaseCount=elements.reduce((s,e)=>s+Object.values(e.releases||{}).filter(Boolean).length,0),semiRigidConnectionCount=elements.reduce((s,e)=>s+Object.values(e.rotationalSprings||{}).filter(v=>v!==null&&v!==undefined&&Number(v)>0).length,0),springCount=(p.nodeSprings||[]).length;
  return{type:'frame3d-buckling',dimension:'3d',solverVersion:'0.27.0',scenario:resolved.scenario,reference:{elementAxialForces:[...axialMap].map(([elementId,N])=>({elementId,N})),loadPattern:'resolved scenario'},dofs:nd,freeDofs:free.length,eigenIterations:eig.iterations,modes,criticalFactor:modes[0].factor,stability:{formulation:'linear-eigen-buckling-3d with released/semi-rigid end connections + spatial nodal springs in elastic K only',connectionAware:true,releaseCount,semiRigidConnectionCount,springCount,springsInGeometricStiffness:false,convention:'N>0 tension; compression negative'}};
}
