...
export function solveBuckling3D(project,scenarioId,options={}){
  ...
  return{type:'frame3d-buckling',dimension:'3d',solverVersion:'0.27.0',scenario:resolved.scenario,reference:{elementAxialForces:[...axialMap].map(([elementId,N])=>({elementId,N})),loadPattern:'resolved scenario'},dofs:nd,freeDofs:free.length,eigenIterations:eig.iterations,modes,criticalFactor:modes[0].factor,stability:{formulation:'linear-eigen-buckling-3d with massless released/semi-rigid end-connection kinematic projection',connectionAware:true,releaseCount,semiRigidConnectionCount,convention:'N>0 tension; compression negative'}};
}
