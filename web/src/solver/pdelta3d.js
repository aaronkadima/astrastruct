...
function prepareFrame3D(project,e,a,b){
  ...
  if(unsupported.length) throw new Error(`P-Delta 3D v0.28: ${e.id} possui carga de barra ainda não suportada (${unsupported[0].kind || 'tipo desconhecido'}).`);
...
export function solveFramePDelta3D(project,options={}){
  ...
  if(elements.some(e => e.type !== 'frame3d')) throw new Error('P-Delta 3D v0.28 suporta somente modelos compostos por elementos frame3d.');
  if(options.initialImperfection || project.settings?.imperfection?.enabled) throw new Error('P-Delta 3D v0.28 ainda não suporta imperfeição geométrica modal.');
  ...
  const prescribed = prescribedSupportDofs(project,map);
  ...
  for(let iteration=1; iteration<=maxIterations; iteration++){
    ...
    for(const item of cache){
      ...
      const kg = matMul(transpose(item.prepared.T),matMul(ktLocal,item.prepared.T));
      addSub(K,kg,item.idx);
    ...
    const forces = cache.map(item => {
      const ug = item.idx.map(i => u[i]), {ul,q} = recoverFrame(item.prepared,ug);
      return {elementId:item.e.id,type:'frame3d',N1:q[0],Vy1:q[1],Vz1:q[2],T1:q[3],My1:q[4],Mz1:q[5],N2:q[6],Vy2:q[7],Vz2:q[8],T2:q[9],My2:q[10],Mz2:q[11],localDisplacements:ul,localAxes:item.prepared.axes,properties:item.prepared.properties,loadSummary:item.prepared.loadSummary};
    });
  ...
  return {type:'frame3d-pdelta',dimension:'3d',analysisType:'pdelta',solverVersion:'0.28.0',dofs:nd,activeDofs:last.free.length,displacements,reactions,elementForces:last.forces,pDelta:{converged:true,iterations:last.iteration,tolerance,maxDelta,axialForces:[...last.axial].map(([elementId,N]) => ({elementId,N})),formulation:'elastic-frame3d + consistent geometric stiffness in both bending planes',convention:'N>0 tension; compression negative'}};
}
