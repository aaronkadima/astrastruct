...
function icon(name){
  return `<svg class="astra-icon" viewBox="0 0 24 24" aria-hidden="true" focusable="false">${GLYPHS[name]||GLYPHS.more}</svg>`;
}
...
function decorate(el,name,label){
  ...
  el.dataset.astraIcon=name;
  el.dataset.astraLabel=label;
  el.setAttribute('aria-label',label);
  el.setAttribute('title',label);
  el.innerHTML=icon(name)+`<span class="sr-only">${label}</span>`;
...
function decorateControls(){
  ...
  document.querySelectorAll('.toolbar [data-tool]').forEach(el=>{
    const meta=TOOL[el.dataset.tool];if(meta)decorate(el,meta[0],meta[1]);
  });
  const v=document.querySelector('.brand small');if(v&&v.textContent.trim()!=='v0.11')v.textContent=' v0.11';
  document.querySelectorAll('.mode-pill').forEach(p=>{if(p.textContent.includes('solver '))p.textContent='solver 0.11.0'});
}
...
function ensureMobileShell(){
  ...
  if(!scrim){scrim=document.createElement('button');scrim.id='mobile-scrim';scrim.className='mobile-scrim';scrim.setAttribute('aria-label','Fechar painel');scrim.onclick=()=>closeMobile(app);app.appendChild(scrim)}
  let dock=document.getElementById('mobile-dock');
  if(!dock){
    dock=document.createElement('nav');dock.id='mobile-dock';dock.className='mobile-dock';dock.setAttribute('aria-label','Navegação móvel AstraStruct');
    dock.innerHTML=`<button data-mobile="library" aria-label="Biblioteca" title="Biblioteca">${icon('library')}</button><button data-mobile="canvas" aria-label="Modelo" title="Modelo">${icon('canvas')}</button><button data-mobile="inspector" aria-label="Inspector" title="Inspector">${icon('inspector')}</button><button data-mobile="results" aria-label="Resultados" title="Resultados">${icon('results')}</button><button data-mobile="more" aria-label="Mais comandos" title="Mais comandos">${icon('more')}</button>`;
    app.appendChild(dock);
    dock.querySelectorAll('[data-mobile]').forEach(b=>b.onclick=()=>{
      const action=b.dataset.mobile;
      if(action==='canvas'){closeMobile(app);return}
      if(action==='library'){const on=!app.classList.contains('mobile-library-open');closeMobile(app);if(on)app.classList.add('mobile-library-open')}
      if(action==='inspector'){const on=!app.classList.contains('mobile-inspector-open');closeMobile(app);if(on)app.classList.add('mobile-inspector-open')}
      if(action==='results'){const on=!app.classList.contains('mobile-results-open');closeMobile(app);if(on)app.classList.add('mobile-results-open')}
      if(action==='more'){const on=!app.classList.contains('mobile-menu-open');closeMobile(app);if(on)app.classList.add('mobile-menu-open')}
      updateDock(app);
    });
  }
  ensureCommandSheet(app);
  updateDock(app);
}

function updateDock(app){
  const states={library:'mobile-library-open',inspector:'mobile-inspector-open',results:'mobile-results-open',more:'mobile-menu-open'};
  document.querySelectorAll('#mobile-dock [data-mobile]').forEach(b=>b.classList.toggle('active',!!states[b.dataset.mobile]&&app.classList.contains(states[b.dataset.mobile])));
}

function ensureCommandSheet(app){
  let sheet=document.getElementById('mobile-command-sheet');
  if(!sheet){sheet=document.createElement('section');sheet.id='mobile-command-sheet';sheet.className='mobile-command-sheet';sheet.setAttribute('aria-label','Comandos AstraStruct');app.appendChild(sheet)}
  const commands=[...document.querySelectorAll('.topbar button[id]')].filter(b=>TOP[b.id]);
  const signature=commands.map(b=>b.id).join('|');if(sheet.dataset.signature===signature)return;sheet.dataset.signature=signature;
  ...
  sheet.querySelector('#mobile-sheet-close').onclick=()=>{app.classList.remove('mobile-menu-open');updateDock(app)};
  sheet.querySelectorAll('[data-trigger]').forEach(b=>b.onclick=()=>{document.getElementById(b.dataset.trigger)?.click();app.classList.remove('mobile-menu-open');updateDock(app)});
}

let queued=false;
function enhance(){decorateControls();ensureMobileShell()}
