...
function stressChart(response){
  ...
  const W=980,H=390,L=78,R=32,T=48,B=58,xmax=Math.max(...pts.map(p=>p.x)),ys=pts.flatMap(p=>[p.sigmaTop,p.sigmaBottom]),ymin=Math.min(...ys,0),ymax=Math.max(...ys,0),span=Math.max(ymax-ymin,1e-9),lo=ymin-.14*span,hi=ymax+.14*span;
  ...
  const ticks=[0,.25,.5,.75,1].map(t=>{const x=L+t*(W-L-R),v=t*xmax;return `<line class="chart-grid" x1="${x}" y1="${T}" x2="${x}" y2="${H-B}"/><text class="chart-label" x="${x}" y="${H-20}" text-anchor="middle">${fmt(v,2)}</text>`}).join('');
  const yticks=[lo,(lo+hi)/2,hi].map(v=>{const y=sy(v);return `<line class="chart-grid" x1="${L}" y1="${y}" x2="${W-R}" y2="${y}"/><text class="chart-label" x="${L-10}" y="${y+4}" text-anchor="end">${fmt(v,2)}</text>`}).join('');
  return `<svg class="stress-chart" viewBox="0 0 ${W} ${H}" role="img" aria-label="Mapa de tensões elásticas ao longo do elemento"><rect class="chart-bg" x="${L}" y="${T}" width="${W-L-R}" height="${H-T-B}"/>${ticks}${yticks}<line class="chart-axis" x1="${L}" y1="${T}" x2="${L}" y2="${H-B}"/><line class="chart-axis" x1="${L}" y1="${H-B}" x2="${W-R}" y2="${H-B}"/>${y0>=T&&y0<=H-B?`<line class="chart-zero" x1="${L}" y1="${y0}" x2="${W-R}" y2="${y0}"/>`:''}<path class="stress-top" d="${path('sigmaTop')}"/><path class="stress-bottom" d="${path('sigmaBottom')}"/><text class="chart-axis-title" x="${(L+W-R)/2}" y="${H-4}" text-anchor="middle">Posição x [m]</text><text class="chart-axis-title" transform="translate(20 ${(T+H-B)/2}) rotate(-90)" text-anchor="middle">Tensão [MPa]</text><text class="stress-legend top" x="${L+14}" y="${T+24}">σ topo</text><text class="stress-legend bottom" x="${L+100}" y="${T+24}">σ base</text></svg>`;
}
